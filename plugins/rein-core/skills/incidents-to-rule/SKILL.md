...
> Helper path 는 plugin install 환경에서는 plugin root 의 `scripts/`, 메인테이너 dogfood 에서는 repo `scripts/` 가 fallback 으로 사용된다. 본문 예시는 아래 resolver block 으로 `$REIN_PLUGIN_ROOT` 를 먼저 정해두고 그 뒤 `"$REIN_PLUGIN_ROOT/scripts/..."` 형태로 호출한다 (Claude Bash 도구 환경에서는 plugin root env 가 unset 일 수 있으므로 매 호출 직전에 다시 해소). 메인테이너 환경에서는 hook 호출 시 RES-1 resolver 가 자동 fallback 하고, 수동 Bash 호출이라면 `scripts/rein-mark-incident-processed.py` 로 그대로 가능.
...
> plugin root 환경 변수는 `/plugin install rein@rein` 활성 세션에서 자동 주입된다. unset 인 메인테이너 dogfood 환경 또는 Claude Bash 도구 환경에서는 위 resolver block 으로 `$REIN_PLUGIN_ROOT` 를 해소하거나, `scripts/rein-mark-incident-processed.py` 로 직접 호출한다 (RES-1 resolver 가 fallback 으로 동일 파일 가리킴).
...
# plugin root 해소 (Claude Bash 도구 환경에서는 ${CLAUDE_PLUGIN_ROOT} 가 unset)
REIN_PLUGIN_ROOT="$(claude plugin path rein-core 2>/dev/null || echo "$HOME/.claude/plugins/marketplaces/rein/plugins/rein-core")"
python3 "$REIN_PLUGIN_ROOT/scripts/rein-aggregate-incidents.py" --count-pending
```

> 메인테이너 환경 (plugin root env unset) 에서는 `scripts/rein-aggregate-incidents.py` 로 직접 호출 가능.
