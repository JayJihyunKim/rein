...
> Helper path 는 plugin install 환경에서는 `${CLAUDE_PLUGIN_ROOT}/scripts/`, 메인테이너 dogfood 에서는 `scripts/` 가 fallback 으로 사용된다. 본문 예시는 plugin path 를 명시 — 메인테이너 환경에서는 `${CLAUDE_PLUGIN_ROOT}` 가 unset 이라도 hook 호출 시 RES-1 resolver 가 자동 fallback. 수동 호출이라면 unset 환경에서 `scripts/rein-mark-incident-processed.py` 로 그대로 가능.

```bash
# status 는 바꾸지 않고 분류만 기록
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/rein-mark-incident-processed.py" \
...
> `${CLAUDE_PLUGIN_ROOT}` 는 `/plugin install rein@rein` 활성 환경에서 자동 주입된다. unset 인 메인테이너 dogfood 환경에서는 `scripts/rein-mark-incident-processed.py` 로 직접 호출 (RES-1 resolver 가 fallback 으로 동일 파일 가리킴).
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/rein-aggregate-incidents.py" --count-pending
```

> 메인테이너 환경 (`${CLAUDE_PLUGIN_ROOT}` unset) 에서는 `scripts/rein-aggregate-incidents.py` 로 직접 호출 가능.
