---
name: codex-review
description: "Codex CLI 코드 리뷰 모드. PASS 시 v2 code_review 증거 발급, severity escalation, Sonnet fallback, 재리뷰는 후속 회차 요청서 양식. rein 의 코드리뷰 게이트 통과 증거를 발급하는 유일한 스킬."
---
...
| Resume --last | 래퍼는 매번 새 세션 — 맥락은 후속 회차 요청서 양식으로 (§2, §7)           | **금지** — 매번 새 세션                                                                |
...
1. **직전 검토 근거** — 같은 사이클의 몇 번째 확인인지, 직전 판정과 차단급 지적 유무.
2. **열린 지적과 처리** — 지적별로 셋 중 하나: 고침(어디를) / 위험 수용(근거·기록 위치) / 반박(근거).
3. **종결된 지적** — 이미 배제·수용해 다시 다루지 않을 항목. 새 세션은 이를 모르므로 적지 않으면 같은 지적이 회차마다 되돌아온다.
4. **직전 검토 이후 바뀐 것** — 항목으로 나열(수정분의 범위).
5. **영향받는 계약과 증거** — 4 가 닿는 계약과 그 검증 명령(`[EVIDENCE]` 블록, §4.1).

요청 문장은 다음 취지로 쓴다: "후속 확인이다 — 필수 검사 축은 그대로 수행하되 4·5 를 우선 확인해 달라. 중대한 결함은 범위와 무관하게 보고해 달라." **"그 밖의 새 결함도 함께 봐 달라", "수정이 새 충돌을 만들지 않았는지 전체를 확인해 달라" 같은 문장은 쓰지 않는다** — 매 회차 전체 재탐색을 요청하는 것과 같다.

**전체 검토로 복귀하는 조건** — 하나라도 해당하면 이 양식을 쓰지 말고 초회와 같은 전체 검토를 요청한다: 새 인터페이스(호출 규약·출력 형식·파일 위치) 추가, 보안 경계(아래 (b) 의 범주 — 한정 열거 아님) 변경, 작업 범위(DoD·Scope 항목) 변경, 직전 검토 이후 변경이 검토 대상의 대부분을 차지.

**함께 지킬 운영 규율**:

- 작업 기준서(DoD)의 범위를 **검토 단위에 맞춘다**. 기준서가 아직 구현하지 않은 범위 항목까지 가리키면 매 회차 그 항목들이 다시 평가된다.
- 문서 리뷰에서 "구현 단계 이관" 참고사항과 오류 경로·상태 수명 지적을 **문서 본문의 계약으로 편입하지 않는다**(§6.7). 편입하면 다음 회차에 "문서 내부 충돌" 로 되돌아오고, 그 지적은 판정에 반영된다.
- 후속 회차의 강도는 **조건부 기본값**이다 — 아래 셋이 모두 맞으면 `[EFFORT:medium]` 으로 부른다(§6.1): (a) 직전 독립 검토 보고에 High 가 없고 필수 검사 축이 수행됐다, (b) 변경이 직전 지적 처리에 한정되고 새 인터페이스·보안 경계·범위 변경이 없다 — 보안 경계는 보안 규칙이 다루는 범주 전부다(입력 검증·경로·권한·비밀 취급·인증·세션·TLS·인증서·암호화·역직렬화·코드 실행·외부 신뢰 경계 등, 한정 열거가 아니다) 또는 §6.3 floor 경로, (c) 새 판단이 필요한 변경이 없다. "High 를 고쳤다" 는 (a) 충족이 아니다 — 직전 보고에 High 가 있었으면 그 회차는 대상 밖이다. 셋 중 하나라도 어긋나면 medium 을 쓰지 않는다: 직전 보고에 High 가 있었거나(그 High 를 고쳤더라도 — 그 수정은 아직 독립 검토를 받지 않았다) (b)·(c) 에 어긋나면 §6.3 재승급 규칙대로 `[EFFORT:high]` 를 넣고, 직전 보고가 필수 축을 수행하지 않았으면 그 검토는 완결이 아니므로 마커 없이 전체 검토와 산출 강도(§6.3, 위험 경로 floor 포함)로 복귀한다. 이 기본값은 medium 이 놓치는 결함 폭을 재지 않은 운영 가설이며, 이후 리뷰·CI·버그에서 드러난 누락과 대조한다.
- 회차 상한(§10)과 별개로, 누적 대기가 길어지면 재호출 대신 사람에게 **남은 위험 수용 / 범위 축소 / 중단** 중 결정을 요청한다. 시간이 많이 들었다는 사실은 통과 사유가 아니다.

**한계**: 래퍼는 후속 회차에도 검사 축 전부를 응답에 요구한다. 이 양식은 범위 축소를 **보장하지 않는다**. 효과를 볼 때는 소요 시간만이 아니라 놓친 결함도 함께 본다 — `trail/review-events/` 의 소요는 판정으로 끝난 호출만 담으므로 총 대기 지표가 아니다.
...
| 코드 리뷰 (fix 후 재리뷰, 같은 사이클) | `read-only`       | 래퍼 재호출(새 세션) + 후속 회차 요청서 양식 (§2) |
...
**문서 지적의 차단 기준** (래퍼가 두 봉투 — 문서 봉투의 판정 규율 (iv), 코드 봉투의 Claim Audit — 에 같은 문장으로 방출한다): 수정 필요(NEEDS-FIX)로 이어지는 지적은 결정·범위·사실의 오류여야 하고, 무엇을 잘못 전달하는지와 그 근거가 지적에 있어야 한다. 같은 뜻의 표현 선택·배치·명칭 취향은 "참고사항" 으로 분리되며 verdict 를 승격시키지 않는다 — 참고사항은 위 표의 다음 행동을 결정하지 않는다. 실질 결함(잘못된 사용법, 영향 범위 과장, 필수 보안·안전 제약의 누락 — 인증·TLS 검증·안전한 역직렬화 등은 범위의 오류이며 "구현 단계 이관" 참고사항으로 돌리지 않는다 —, 완료 주장, 수치 불일치, 누락된 추적 연결)은 계속 차단이다. Scope ID 의 측정 가능성도 문자열 형식이 아니라 Scope Items 표의 설명·검증 방법까지 읽어 판정한다 — 세 요소(대상·방향·상황)가 설명에 있으면 문자열만으로 플래그하지 않고, 설명에도 없을 때만 Medium 이다.

**sonnet 셀프리뷰**: 변경 diff 를 직접 확인하고, PASS 판정이면 §5 발급을 그대로 수행한다 (발급 자체에는 reviewer 구분 필드가 없다 — 사람 보고 문구에서만 "self-review" 로 구분한다).

Round 관리: 회차는 래퍼가 사이클 단위로 계수하며 상한을 **코드로 강제**한다 (§10). 같은 사이클 내 재리뷰도 래퍼는 새 세션으로 실행한다 — 이전 검토의 맥락은 호출자가 후속 회차 요청서 양식(§2)으로 전달한다 (§7).
...
exit 5 앵커도 동일 원리로 **라인 시작 anchored — substring 검색 금지**: 래퍼가 스풀 내용을 방출하는 모든 경로에서 방출 전에 라인 시작의 예약 접두사 `ERROR: [codex-review][review-timeout]` 을 `ERROR: [codex-review][review-…]` 로 소독하므로, 결합 로그(`> log 2>&1`)에서 라인 앵커 매치는 래퍼 소유 stderr 라인뿐이다 (codex 자체 출력의 리터럴 오염으로 인한 오분류 배제).

### 4.3 하니스 600s 상호작용 (워치독 생존 계약)

- 워치독은 **래퍼 프로세스 내부** 메커니즘 — 하니스(Claude Code Bash)가 래퍼 프로세스를 살려두는 동안만 유효하다. 하니스가 Bash timeout 으로 래퍼를 죽이거나 process group 을 정리하면 워치독 생존은 **비보장** — 이 경우 exit 5 계약은 발화하지 않고, 호출자는 하니스 timeout 을 기존 실행 실패로 처리한다 (§4 본문).
- **권장 호출 패턴**: `run_in_background: false` + Bash `timeout: 600000`(max) + 출력 직접 파일 리다이렉트 (`> /tmp/<output>.log 2>&1` — background-jobs.md 형식) 유지.
- 절대 상한이 없으므로 정당한 장기 리뷰는 600s 를 넘을 수 있다. 600s 를 상회하는 리뷰가 하니스에 잘리는 것은 기존 "600s 한계 넘으면 prompt 쪼개기" 규약의 영역 — 워치독이 해소하는 것은 "행 방치" 이지 "하니스 상한" 이 아니다.
...
- **재승급 불변식 (E5)**: 재리뷰의 high 승급은 회차 숫자가 아니라 **남은 위험**으로 정한다 — 직전 회차 보고에 High 가 있었거나(그 High 를 고쳤더라도 — 수정은 아직 독립 검토 전이다) 후속 회차가 §2 (b)·(c) 에 어긋나면(지적 처리를 넘는 새 인터페이스·보안 경계·범위 변경, 새 판단) caller 가 `[EFFORT:high]` 마커를 주입한다. §2 후속 회차 조건 (a)~(c) 를 모두 만족하면 조건부 기본값 `[EFFORT:medium]`, 직전 보고가 필수 축을 수행하지 않았으면(직전 검토가 완결이 아님) 마커 없이 산출 강도(floor 포함)로 전체 검토한다. 마커는 산출보다 우선이므로 어떤 산출 결과도 재승급된 high 를 약화시키지 않는다.
...
같은 리뷰 사이클 내 재리뷰는 모델을 바꾸지 않는다 — effort 는 §2 의 후속 회차 규칙과 이 절의 재승급 규칙을 따른다. 게이트 모델이 `CODE_GATE_MODEL` 단일 고정이므로 이 일관성은 **구조적으로 보장**된다 — 규모/회차별 모델 혼용은 결함 탐지 성향·severity 보정·PASS 기준이 달라져 게이트 판정 비일관을 만들므로 하지 않는다.
...
**문서 리뷰의 판정 축 (2026-08-04)**: spec-review 모드는 코드 리뷰와 **다른 슬롯 세트**를 받는다. 래퍼가 결정 건전성 / 범위·추적성 / 주장 감사 3개 슬롯과 판정 규율 4종을 방출하며, 코드 결함·테스트 정합 슬롯은 방출하지 않는다.
...
- 수정 필요로 이어지는 지적은 **결정·범위·사실의 오류**여야 하고, 무엇을 잘못 전달하는지와 근거가 지적에 있어야 한다. 같은 뜻의 표현 선택·배치·명칭 취향은 "참고사항" 으로 분리해 verdict 를 승격시키지 않는다 — 실질 결함(잘못된 사용법·영향 범위·필수 보안·안전 제약의 누락(범위의 오류 — "구현 단계 이관" 참고사항이 아니다)·완료 주장·수치 불일치·누락된 추적 연결)은 계속 차단한다. 코드 봉투의 Claim Audit 도 문서 주장에 같은 문장을 쓴다 (§3).
...
**래퍼는 세션을 이어가지 않는다.** `scripts/rein-codex-review.sh` 는 초회든 재리뷰든 매번 새 `codex exec` 세션을 띄운다 — 이전 회차의 대화는 다음 회차에 전달되지 않는다. 같은 사이클 재리뷰에서 이전 검토의 맥락을 살리는 수단은 세션 재사용이 아니라 **후속 회차 요청서 양식(§2)** 이다.

수동으로 `codex exec resume --last` 를 쓰는 것은 권하지 않는다 — 래퍼를 거치지 않으면 회차 계수(§10)·이벤트 기록·증거 발급(§5)이 모두 빠진다.

**사이클 경계**: 새 DoD, 새 PR, 새 기능 개발로 전환되면 후속 회차 양식을 쓰지 않고 초회 전체 검토로 시작한다. Second opinion 용도로는 절대 이 스킬이 아니라 `/codex-ask` 를 사용한다 (v2 증거 미발급 + resume 금지).
