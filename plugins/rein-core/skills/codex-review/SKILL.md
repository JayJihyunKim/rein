...
1. 모델은 단일 출처(`plugins/rein-core/config/codex-models.sh` 의 `CODE_MODEL`)가 결정한다. 래퍼가 이 값을 `-m` 으로 전달하므로 수동 모델 선택은 불필요하다 — 모델 rename 시 그 파일만 수정한다.
...
**Wrapper 경로** — `bash scripts/rein-codex-review.sh [--non-interactive] < /tmp/<prompt>.txt`:
...
**모델 설정 에러(종료 코드 3)는 폴백 대상이 아니다.** 래퍼가 exit 3 으로 종료하면 codex 가 모델명을 거부한 것(upstream rename 등)이다. sonnet 폴백으로 넘기면 잘못된 모델 설정이 가려지므로, 폴백하지 말고 `plugins/rein-core/config/codex-models.sh` 의 `CODE_MODEL` 을 codex 의 최신 모델명으로 갱신하도록 사용자에게 안내한다. 래퍼가 경로와 현재 값을 stderr 로 출력한다.
...
```
# 모델은 래퍼가 단일 출처(codex-models.sh)의 CODE_MODEL 을 -m 으로 전달.
codex exec -m "$CODE_MODEL" --config model_reasoning_effort="high" --sandbox read-only \
