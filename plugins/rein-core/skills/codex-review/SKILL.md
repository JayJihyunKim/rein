...
> **Pipe vs file redirect**: `printf '...' | bash wrapper.sh` 패턴은 pre-bash-guard 가
...
1. Ask the user (via `AskUserQuestion`) which model to run: `gpt-5.5` (default, config.toml 기본값) or `gpt-5.3-codex`.
...
- wrapper 는 stdin 으로 prompt 를 받는다. **항상 file redirect (`< /tmp/<file>`)** 사용 — pipe (`printf '...' | bash wrapper.sh`) 는 pre-bash-guard 가 차단 (stdin 명령 source 검증 불가)
