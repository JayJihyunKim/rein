...
**전체 검토로 복귀하는 조건** — 하나라도 해당하면 이 양식을 쓰지 말고 초회와 같은 전체 검토를 요청한다: 새 인터페이스(호출 규약·출력 형식·파일 위치) 추가, 보안 경계(입력 검증·경로·권한·비밀) 변경, 작업 범위(DoD·Scope 항목) 변경, 직전 검토 이후 변경이 검토 대상의 대부분을 차지.
...
- 차단급 지적이 남아 있지 않은 후속 확인에 한해 `[EFFORT:medium]` 을 시도할 수 있다(§6.1). 기본값이 아니라 시험값이다 — 3회차 재승급 규칙(§6.3)과 부딪히면 그 규칙이 우선한다.
