...
2. 모델 / 강도는 단일 출처(`plugins/rein-core/config/codex-models.sh`)에서 읽는다 — 분석 역할은 **3계층 프로필** 중 하나를 질문 성격으로 선택한다. 모델을 사용자에게 묻지 않는다(역할 고정 + tier 는 호출자 판단). 사용자가 강도를 명시하면 그 값을 존중한다(기존 규칙 유지).

   ### 3계층 프로필 선택

   | tier | 용도 판정 기준 | 변수 |
   |---|---|---|
   | **fast** | 기계적 추출·분류·요약 (판단 여지 낮음) | `ANALYSIS_FAST_MODEL` / `ANALYSIS_FAST_EFFORT` |
   | **default** | 일반 second opinion — **판단이 애매하면 이 tier** | `ANALYSIS_DEFAULT_MODEL` / `ANALYSIS_DEFAULT_EFFORT` |
   | **deep** | 고위험·모호 판단, 설계 반박, 보안/게이트 관련 | `ANALYSIS_DEEP_MODEL` / `ANALYSIS_DEEP_EFFORT` |

   - 호출자(Claude)가 질문 성격으로 tier 를 선택한다. 어느 tier 인지 판단이 안 서면 **default**.
   - **ultra 는 기본 미사용** — 사용자가 명시적으로 요청한 경우에만 허용하며, 그 경우에도 도장 미생성(second opinion 본질, §3) 불변.
   - legacy alias `ANALYSIS_MODEL`/`ANALYSIS_EFFORT` 는 default tier 를 가리키는 하위 호환 참조일 뿐이다 — 신규 호출은 tier 변수를 직접 쓴다.
3. Sandbox 는 항상 `--sandbox read-only`. Second opinion 은 파일 편집 권한이 불필요하다.
4. **새 `codex exec` 세션** 으로 실행 (resume 금지 — §4 참조):
   ```bash
   # 모델명은 codex-models.sh 한 곳에서 관리 (모델 rename 시 그 파일만 수정).
   source "${CLAUDE_PLUGIN_ROOT:-plugins/rein-core}/config/codex-models.sh"
   # tier 선택: 질문 성격에 따라 FAST / DEFAULT / DEEP 중 하나 (애매하면 DEFAULT).
   codex exec \
     -m "$ANALYSIS_DEFAULT_MODEL" \
     --config model_reasoning_effort="$ANALYSIS_DEFAULT_EFFORT" \
     --sandbox read-only \
     --full-auto \
     -C <workdir>
   ```
5. 출력을 Claude 주 컨텍스트에 반영 후, 사용자에게 summary + 필요 시 의사결정 질문으로 요약 보고.
6. 추가 질의가 필요하면 **또 다른 새 세션** 으로 시작. 기존 세션 resume 하지 않는다.
...
```bash
source "${CLAUDE_PLUGIN_ROOT:-plugins/rein-core}/config/codex-models.sh"
# tier 는 질문 성격에 따라 선택 (§2 3계층 프로필 선택 표) — 예시는 default tier.
codex exec -m "$ANALYSIS_DEFAULT_MODEL" --config model_reasoning_effort="$ANALYSIS_DEFAULT_EFFORT" \
