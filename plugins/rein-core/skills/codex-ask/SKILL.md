...
2. 모델 / 강도는 단일 출처(`plugins/rein-core/config/codex-models.sh`)에서 읽는다 — 분석 역할이므로 `ANALYSIS_MODEL` / `ANALYSIS_EFFORT` 를 사용한다. 모델을 사용자에게 묻지 않는다(역할 고정). 사용자가 강도를 명시하면 그 값을 존중한다.
...
4. **새 `codex exec` 세션** 으로 실행 (resume 금지 — §4 참조):
   ...
   codex exec \
     -m "$ANALYSIS_MODEL" \
     --config model_reasoning_effort="$ANALYSIS_EFFORT" \
     --sandbox read-only \
     --full-auto \
     -C <workdir>
   ```
...
```bash
source "${CLAUDE_PLUGIN_ROOT:-plugins/rein-core}/config/codex-models.sh"
codex exec -m "$ANALYSIS_MODEL" --config model_reasoning_effort="$ANALYSIS_EFFORT" \
...
- **모델 거부 감지 (fail-soft)**: codex 출력에 `is not supported` / `invalid_request_error` / `model_not_found` 가 보이면 모델명이 변경된 것이다(codex upstream rename). `${CLAUDE_PLUGIN_ROOT:-plugins/rein-core}/config/codex-models.sh` 의 `ANALYSIS_MODEL` 을 최신 모델명으로 갱신하도록 사용자에게 안내하고 재시도한다. (second opinion 은 게이트가 아니므로 작업 차단은 없다.)
