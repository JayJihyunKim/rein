...
Maintainer-only excludes (path-suffix match, not full path):
  ...
  v1.0.4 (2026-05-11): domain-skill exclusion (stitch-design / stitch-loop /
  taste-design / design-md / enhance-prompt / react-components / remotion /
  shadcn-ui) removed — domain plugins (`plugins/rein-stitch`/`-react`/`-remotion`)
  decommissioned and their skill subtrees deleted from both trees.
...
# Domain-skill exclusion removed in v1.0.4 — domain plugins decommissioned.
# If a future domain plugin is reintroduced, restore the frozenset here and
# document the new exclusion in branch-strategy.md.
