...
def _resolve_inventory_dir() -> Path:
    ...
    if plugin_root:
        ...
        if candidate.exists():
            resolver = candidate
    if resolver is None:
        repo_resolver = Path("plugins/rein-core/scripts/rein-state-paths.py")
        opt_in = Path(".rein/project.json").exists() or Path(".rein/cache").is_dir()
    ...
    if resolver is not None:
        try:
            ...
            raise
    return Path(".claude/cache")


CACHE_DIR = _resolve_inventory_dir()
