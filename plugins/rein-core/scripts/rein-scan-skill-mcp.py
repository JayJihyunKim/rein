...
def _resolve_inventory_dir(project: Path) -> Path:
    """Return the inventory directory, mode-aware.

    Mirrors rein-generate-skill-mcp-guide.py:_resolve_inventory_dir(). The
    scanner accepts an explicit `project` because it is invoked with
    `--project-dir` whereas the generator runs from cwd. Resolution order:

      1. Plugin install — ${CLAUDE_PLUGIN_ROOT}/scripts/rein-state-paths.py
      2. rein-dev source checkout with explicit opt-in
         (`.rein/project.json` or `.rein/cache/` present under `project`) —
         repo-local resolver at `plugins/rein-core/scripts/rein-state-paths.py`.
      3. Legacy fallback — `<project>/.claude/cache/` (pre-migrate clones).

    The resolver fail-closes (re-raises) on plugin mode when
    `CLAUDE_PLUGIN_DATA` is missing, so the scanner write path stays in
    lockstep with the generator's read path and no silent fallback to
    `.claude/cache` occurs in plugin mode.
    """
    resolver: Path | None = None
    plugin_root = os.environ.get("CLAUDE_PLUGIN_ROOT")
    if plugin_root:
        candidate = Path(plugin_root) / "scripts" / "rein-state-paths.py"
        if candidate.exists():
            resolver = candidate
    if resolver is None:
        repo_resolver = project / "plugins/rein-core/scripts/rein-state-paths.py"
        opt_in = (project / ".rein/project.json").exists() or (project / ".rein/cache").is_dir()
        if opt_in and repo_resolver.exists():
            resolver = repo_resolver
    if resolver is not None:
        try:
            out = subprocess.run(
                [sys.executable, str(resolver), "inventory"],
                capture_output=True,
                text=True,
                check=True,
            )
            ...
            # Resolver fail-closes on plugin mode + missing CLAUDE_PLUGIN_DATA.
            # We propagate by re-raising so the caller surfaces a clear error
            # instead of silently writing to legacy .claude/cache/.
            raise
    return project / ".claude/cache"
...
def main():
    ...
    cache_dir = _resolve_inventory_dir(project)
