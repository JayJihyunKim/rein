...
# honoring rein-state-paths.py mode resolution (Phase 3 Task 3.5).
#
# Resolution order (matches other state file resolvers — Phase 3):
#   1. ${CLAUDE_PLUGIN_ROOT}/scripts/rein-state-paths.py active-dod-choice-log
#      when the plugin install root is set + resolver present.
#   2. <repo>/plugins/rein-core/scripts/rein-state-paths.py with explicit opt-in
#      (.rein/project.json or .rein/cache/ already present).
#   3. Legacy fallback ``.claude/cache/active-dod-choice.log`` so existing
#      rein-dev clones keep writing to the historical location until
#      ``rein migrate`` runs in Phase 4.
_sad_resolve_choice_log_path() {
  local resolver=""
  if [ -n "${CLAUDE_PLUGIN_ROOT:-}" ] && \
     [ -f "$CLAUDE_PLUGIN_ROOT/scripts/rein-state-paths.py" ]; then
    resolver="$CLAUDE_PLUGIN_ROOT/scripts/rein-state-paths.py"
  fi
  if [ -z "$resolver" ] && [ -f ".rein/project.json" ] && \
     [ -f "plugins/rein-core/scripts/rein-state-paths.py" ]; then
    resolver="plugins/rein-core/scripts/rein-state-paths.py"
  fi
  if [ -z "$resolver" ] && [ -d ".rein/cache" ] && \
     [ -f "plugins/rein-core/scripts/rein-state-paths.py" ]; then
    resolver="plugins/rein-core/scripts/rein-state-paths.py"
  fi
  if [ -n "$resolver" ]; then
    local resolved
    resolved=$(python3 "$resolver" active-dod-choice-log 2>/dev/null || true)
    if [ -n "$resolved" ]; then
      printf '%s\n' "$resolved"
      return 0
    fi
  fi
  printf '%s\n' ".claude/cache/active-dod-choice.log"
}
...
# the per-session flag lives next to the resolved log so the cleanup hook
# (stop-session-gate.sh) and the migrator move both together.
_sad_record_session_choice() {
  local tier="$1"
  local path="$2"
  local reason="$3"
  local log
  log="$(_sad_resolve_choice_log_path)"
  local cache_dir
  cache_dir="$(dirname "$log")"
