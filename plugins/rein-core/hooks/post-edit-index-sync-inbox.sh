...
post_edit_policy_gate "post-edit-index-sync-inbox"
...
# Ancestry check (codex review 2026-04-29 Medium):
# Only act when the edited file is inside the resolved PROJECT_DIR. If the
# helper picked an unrelated repo (e.g. invocation from a hostile cwd), the
# absolute FILE_PATH won't be a descendant of PROJECT_DIR and we silently
# exit — preserving the original "broken layout = silent skip" contract.
# Relative paths are also rejected: we cannot verify ancestry without
# canonicalizing against an unknown cwd, so fail-closed is safer than
# accidentally writing inbox state for the wrong project (codex round 2).
case "$FILE_PATH" in
  "$PROJECT_DIR"/*) ;;  # absolute path inside PROJECT_DIR
  *) exit 0 ;;          # relative path or absolute outside — silent skip
esac
