...
incident_advisory_check() {
  ...
  # Auto mode: silence the repeat-pattern advisory loop.
  _auto_silent=0
...
METAEOF
      ...
      # Auto mode: skip the 3-times-blocked escalation block.
      if [ -f "${CLAUDE_PLUGIN_ROOT:-}/hooks/lib/auto-mode.sh" ]; then
        # shellcheck disable=SC1091
        . "${CLAUDE_PLUGIN_ROOT}/hooks/lib/auto-mode.sh" 2>/dev/null || true
        if declare -F is_auto_mode >/dev/null 2>&1 && is_auto_mode; then
          if declare -F auto_mode_log_bypass >/dev/null 2>&1; then
            auto_mode_log_bypass "stop-session-gate: skip 3x-block (PENDING_COUNT=$PENDING_COUNT)"
          fi
          exit 0
        fi
      fi
      echo "[rein] The session has been blocked more than 3 times because there are unresolved incidents. The decision skill may not be responding. To force the session to close, run: touch trail/dod/.skip-stop-gate" >&2
      if [ -x "$EMIT_PY" ]; then
        python3 "$EMIT_PY" "$PENDING_COUNT"
      else
        echo '{"decision":"block","reason":"There are unresolved incidents that need a decision before the session ends. The emit helper is missing — run /incidents-to-rule or touch trail/dod/.skip-stop-gate to proceed."}'
      fi
      # Claude Code Stop hook: block via JSON decision field, not exit code.
      exit 0
    fi

    # Auto mode: skip the main pending-incident block.
    if [ -f "${CLAUDE_PLUGIN_ROOT:-}/hooks/lib/auto-mode.sh" ]; then
      # shellcheck disable=SC1091
      . "${CLAUDE_PLUGIN_ROOT}/hooks/lib/auto-mode.sh" 2>/dev/null || true
      if declare -F is_auto_mode >/dev/null 2>&1 && is_auto_mode; then
        if declare -F auto_mode_log_bypass >/dev/null 2>&1; then
          auto_mode_log_bypass "stop-session-gate: skip pending-incident block (PENDING_COUNT=$PENDING_COUNT)"
        fi
        exit 0
      fi
    fi
    if [ -x "$EMIT_PY" ]; then
      ...
      echo '{"decision":"block","reason":"There are unresolved incidents that need a decision before the session ends. Run /incidents-to-rule or /incidents-to-agent to process them, then try ending the session again."}'
    fi
    # Claude Code Stop hook: block via JSON decision field, not exit code.
    exit 0
  fi
...
# --- 결과 판정 ---
if [ -n "$MISSING" ]; then
  echo "[rein] The session cannot end yet because some items were not completed." >&2
  echo -e "$MISSING" >&2
  echo "" >&2
  echo "To finish the session, choose one of these options:" >&2
  echo "  1) Edit trail/index.md — if the post-edit-index-sync-inbox hook is installed," >&2
  echo "     today's work log is created automatically (v0.4.1+)." >&2
  echo "  2) Create the log directly in your terminal:" >&2
  echo "     echo '# session note' > trail/inbox/${TODAY}-session.md" >&2
  echo "  3) Emergency exit (an audit entry is recorded): set REIN_BYPASS_STOP_GATE=1" >&2
  echo "     before your next command to bypass this check." >&2
  echo "  4) If you did real work, commit or stage the changes — git activity is" >&2
  echo "     accepted as proof of work and this check passes automatically." >&2
  exit 2
fi
