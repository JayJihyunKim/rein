...
rein 게이트(`exit 2`)에 막히면 **환경을 조작해 통과시키지 않는다**. 금지 행위: 파일 수정시각(mtime) 되돌리기·`touch`, 마커/도장(stamp) 위조·삭제·타임스탬프 편집, hook 비활성화. 차단이 **오탐**으로 보여도 스스로 우회하지 말고 **멈춘다**.

- **정당한 해소만** — 누락 단계 완료, 실제 조건 수정으로만 푼다. 게이트 우회는 reproduction-first 가 잡으려는 root cause 를 숨기는 것과 같다.
- **오탐이면 사용자에게 escalate** — (a) 막힌 파일, (b) 차단 이유, (c) 오탐이라 보는 근거를 평문으로 보고하고 지시를 기다린다. 검토 신선도 오탐(checkout/cherry-pick 으로 수정시각만 밀린 경우)의 정당한 해소는 사용자/메인테이너가 재리뷰(내용 기반 도장) 또는 승인 후 retrospective 재도장으로 처리한다.
...
- **수정 완료**:
  > "[버그명] 을 수정했습니다. 재현 테스트가 통과로 바뀌었고, 기존 테스트도 모두 통과했습니다."
- **원인 미파악**:
  > "재현 테스트는 작성했지만 원인을 아직 좁히지 못했습니다. [현재까지 의심 가는 부분 평문 1~2문장]. 진행 방향을 알려주세요."
- **차단 발생**:
  > "[이유 평문 1문장] 으로 잠시 멈췄습니다. [무엇이 필요한지]."

변경한 파일 경로·재현 테스트 명령어는 사용자가 검증할 수 있어야 하므로 채팅 본문에 그대로 둔다.
