...
- **정당한 해소만** — 누락 단계 완료, 실제 조건 수정으로만 푼다.
...
사용자에게 답변하는 채팅 본문에는 내부 식별자 (`verdict`, `handoff`, `digest`, `evidence`) 를 노출하지 않는다. 평문으로 다음 흐름을 따른다.
...
변경한 파일 경로·테스트 명령어 같이 사용자가 직접 검증·복사할 자료는 채팅 본문에 그대로 둔다 (검증 가능성 보존). 운영 내부 식별자 (digest, Scope ID 등) 는 본문에 쓰지 않는다.
