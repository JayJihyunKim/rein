...
채팅 형식 — **두 층 구조** (communication-improve, 2026-05-28). 평문 요약을 위에 두어 사용자가 빠르게 판단할 수 있게 하고, 상세 라우팅 정보 (어떤 에이전트·스킬·MCP) 는 아래에 두어 승인 정보를 보존한다. agent 선택 자체는 사용자 승인 흐름에 필요하므로 채팅에서 제거하지 않는다.

```
[작업명] 을 진행하겠습니다.
계획: [한 문장 평문 요약 — "무엇을 어떻게" 까지]

이 방향으로 진행할까요? (수정하려면 말씀해 주세요)
...
평문 1줄은 사용자가 가장 먼저 읽는 행. 내부 식별자 (`agent`, `skill`, `mcp`) 라는 레이블이 아니라 "담당 에이전트 / 보조 스킬 / 외부 자료" 로 한국어 라벨링한다. 에이전트/스킬/MCP 자체의 식별 이름 (`rein:feature-builder`, `context7` 등) 은 사용자가 어떤 도구가 선택됐는지 검증할 수 있어야 하므로 그대로 둔다.

차단·실패·복귀 등 운영 상황 보고는 `plugins/rein-core/rules/response-tone.md` 의 보고 문장 구조를 따른다 (방금 한 것 → 결과 → 다음 단계).
