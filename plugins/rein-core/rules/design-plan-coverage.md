...
`scope-id-version` 메타는 design frontmatter 에만 둔다 (plan 은 design ref 로 상속). 미기입 = v1 (legacy 호환). 알 수 없는 값 (`v99` 등) = validator exit 2 (fail-closed, silent downgrade 금지).
...
## 3. DoD 문서 — covers: 필드 + `## 범위 연결` 섹션

### 3.1 covers: 필드 (advisory)

`trail/dod/dod-*.md` 가 특정 plan 의 work unit 을 구현하는 경우, DoD frontmatter 또는 본문에 `covers: [A1, E1]` 필드를 포함하는 것을 **권고한다**. 이 필드는 plan work unit 의 `covers:` 와 일치해야 한다.

### 3.2 `## 범위 연결` 섹션 규격

DoD 는 `## 범위 연결` 섹션을 포함할 수 있다. 이 섹션이 있으면 validator 의 `dod` 서브커맨드가 `covers ⊆ plan.matrix.implemented` 관계를 검증한다.
