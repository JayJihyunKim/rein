...
긴 sync 명령 (pytest, npm test, cargo build, integration E2E, backtest 등) 은 `rein job start <name> -- <cmd>` 로 background 처리. foreground 로 세션 붙잡지 마라. 예외: codex 계열 (codex exec / /codex-review / /codex-ask) 만 foreground + 직접 파일 출력 (`> /tmp/file 2>&1 < /dev/null`) + run_in_background:false. auto-background 전환 시 stdin unix socket 으로 codex sandbox 초기화가 hang 한다. BashOutput 으로 턴 경계 너머 상태 보관 금지.
