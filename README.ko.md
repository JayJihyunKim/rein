...
최신 릴리즈: **v1.4.5** (2026-06-05) — 신규 설치 첫 세션 온보딩 안내 + 세션 상태의 git 사실 자동화. 갓 설치한 저장소의 첫 세션에 짧은 온보딩 안내가 한 번 표시되고, 핵심 게이트 차단 메시지가 "막힌 이유"뿐 아니라 "다음 단계"까지 알려줍니다. 세션 상태(`trail/index.md`)에 git 사실(브랜치 / 커밋·푸시 여부 / 변경 수 / 최신 태그)을 손으로 적지 않습니다 — 훅이 세션 시작·종료마다 git 에서 직접 읽어 자동 블록으로 보여주므로(`.rein/state/git-snapshot.md`, 커밋되지 않는 임시 파일; 로컬 git 만, 네트워크 없음) 기록이 실제와 어긋나지 않습니다. ([CHANGELOG](CHANGELOG.md))
