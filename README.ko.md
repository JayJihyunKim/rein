...
최신 릴리즈: **v1.4.7** (2026-06-09) — 코드 리뷰와 문서(설계서·플랜) 리뷰가 같은 모델(`gpt-5.5`)로 통일됩니다. v1.4.6 에서 codex 모델명을 한 파일로 모으며 역할(코드 리뷰·분석)별로 나눴는데, 이번에는 코드 리뷰와 문서 리뷰를 구분 없이 한 모델로 맞췄습니다. 역할별 변수는 나중에 다시 분리할 수 있도록 두되 값만 같게 했습니다. ([CHANGELOG](CHANGELOG.md))
