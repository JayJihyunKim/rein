...
최신 릴리즈: **v1.5.2** (2026-06-11) — 커밋 전 코드 리뷰가 추측으로 고른(무관할 수 있는) 작업 문서를 근거로 "설계 누락/모순" 차단 경고를 내던 오탐을 수정합니다 — 추측 컨텍스트는 이제 참고용으로 정직하게 표시되고, 그 추측의 근본 원인이던 "정상 작업 표식이 세션 시작마다 잘못 삭제되던 문제"도 함께 고쳤습니다. 또한 아주 큰 설계 리뷰 요청이 코드 리뷰로 오분류되어 코드 게이트 통과 표시를 잘못 만들 수 있던 구멍을 포함, 리뷰 도구 신뢰성 구멍 2건을 봉합했습니다. ([CHANGELOG](CHANGELOG.md))
