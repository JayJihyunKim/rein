...
최신 릴리즈: **v1.5.1** (2026-06-11) — v1.5.0 에서 켜진 페르소나가 실제로는 발동하지 않던 문제를 수정합니다. 세션 시작 시 주입되던 규칙 + 페르소나가 한 덩어리로 커져 잘려나가는 바람에, 페르소나는 물론 일부 운영 규칙 본문까지 어시스턴트에 도달하지 못했습니다. 이제 세션 시작에는 각 규칙의 짧은 요약을 주입하고(전체 본문은 필요 시 읽기), 페르소나는 별도로 분리해 항상 도달하며 매 턴 가볍게 유지됩니다 — 말투에만 적용되고 경고·게이트 차단은 냉정하게 유지되는 원칙은 그대로입니다. ([CHANGELOG](CHANGELOG.md))
