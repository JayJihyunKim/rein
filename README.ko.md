...
최신 릴리즈: **v1.6.1** (2026-07-13) — 코드 리뷰(`/codex-review`) 요청서에 수치·통과 주장을 쓰면 재현 증거 블록(주장/명령/종료코드/출력)이 필요하며, 증거 없는 요청서는 리뷰 비용을 쓰기 전에 거부됩니다. 상태 요약 문서의 길이 규칙이 편집 시점에 강제되고, 아무도 읽지 않던 편집 진단 파일은 더 이상 생성되지 않습니다. ([CHANGELOG](CHANGELOG.md))
