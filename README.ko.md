...
최신 릴리즈: **v1.4.4** (2026-06-04) — 설계 문서를 전용 에이전트 없이 직접 쓰면 안내. `docs/specs/` · `docs/plans/` 문서가 `spec-writer`/`plan-writer` 에이전트를 거치지 않고 인라인으로 작성되면, 편집 직후 도는 훅이 부드러운(차단 아님) 안내를 띄웁니다("전용 작성 경로로 다시 작성하는 편이 안전합니다 — 의도한 수동 작성이면 이 안내는 무시해도 됩니다"). 정상 경로(전용 에이전트 작성)에서는 안내가 뜨지 않고, 차단이 아니므로 수동 수정·리뷰 보정·외부 에디터 작성·마이그레이션은 그대로 동작합니다. 설계 문서를 쓸 때만 발동하므로 매 편집 hot-path 지연 영향은 없습니다. ([CHANGELOG](CHANGELOG.md))
