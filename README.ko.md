...
최신 릴리즈: **v1.5.0** (2026-06-09) — 교체 가능한 어시스턴트 페르소나가 추가되고 기본으로 켜집니다(기본 `boss-ace`, `.rein/policy/persona.yaml` 에 `enabled: false` 로 끄기). 페르소나는 **말투만** 입히고 경고·게이트 차단·운영 정보는 냉정하게 유지합니다(응답 규칙이 항상 우선). 또한 codex 코드 리뷰 게이트의 무결성 결함 여러 건을 수정했습니다 — codex 실행 실패를 통과로 보지 않고, 리뷰 본문에 인용된 판정을 결론으로 오인하지 않으며, 무관한 이전 커밋 대신 실제 미커밋 변경을 리뷰합니다. ([CHANGELOG](CHANGELOG.md))
