...
최신 릴리즈: **v1.5.8** (2026-06-26) — 코드 리뷰(`/codex-review`)가 변경 규모에 맞춰 빨라집니다. 이전에는 자동 리뷰가 거의 항상 가장 깊은(느린) 강도로 돌았지만, 이제 변경 양(줄 수·파일 수)과 문서 길이를 보고 작은 변경은 가볍게, 큰 변경만 깊게 검토합니다. 직접 강도를 지정하면 그 값이 우선합니다. ([CHANGELOG](CHANGELOG.md))
