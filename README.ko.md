...
최신 릴리즈: **v1.5.3** (2026-06-12) — 정직한 에이전트인데도 게이트가 조용히 안 켜지던 4건을 봉합합니다: `git -C <폴더> commit`·공백 두 칸 `git  commit` 같은 형태가 더 이상 리뷰·보안 검사를 건너뛰지 않고, 커밋 메시지에 "git merge"가 들었다는 이유로 검사가 통째로 건너뛰던 문제를 고쳤으며, "설계 먼저, 코드 나중" 요구가 몇몇 디렉토리 이름이 아니라 더 많은 언어·구조(Go `internal`·`cmd`, 저장소 루트 소스)에 적용되고, Python 이 바로 잡히지 않는 환경에서 게이트가 스스로 꺼지지 않고 켜진 상태(fail-closed)를 유지합니다. ([CHANGELOG](CHANGELOG.md))
