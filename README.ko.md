...
최신 릴리즈: **v1.6.5** (2026-08-05) — 코드 리뷰(`/codex-review`)에 회차 상한(기본 5회, 명시 선언·기록되는 연장만 허용)이 생기고, 설계·계획 문서는 코드 리뷰 지시 대신 문서에 맞는 기준으로 판정됩니다. 게이트 오차단 3건도 수리했습니다: 저장소 밖을 가리키는 표식이 편집을 잠그지 않고, 미리뷰 문서는 실제로 그 문서를 참조하는 작업만 차단하며, 스크립트에 기록될 텍스트나 리터럴 경로 샌드박스의 `git commit` 은 실제 커밋으로 오인되지 않습니다(변수 경로·이 저장소의 워크트리는 종전대로 차단). ([CHANGELOG](CHANGELOG.md))

이전: **v1.6.4** (2026-07-27) — 페르소나 목록에서 **새로 만들기**를 바로 고를 수 있고, 페르소나를 바꾸면 그 자리에서 즉시 적용되며, 만들 때 인사말을 직접 고르거나 쓸 수 있습니다. 코드 리뷰(`/codex-review`)가 테스트를 돌리는 중에 잘못 중단되던 문제도 고쳤습니다. ([CHANGELOG](CHANGELOG.md))
