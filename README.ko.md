...
최신 릴리즈: **v2.1.0** (2026-09-07) — 사용자 대면 변화 3건: 리뷰 요청서의 증거 블록 밖에 실행 결과 서술이 남아 있으면 codex 호출 전에 거부하고 걸린 줄과 고쳐 쓰는 방법을 안내 — "테스트 통과" 문장은 항상 거부, 비율·백분율은 계약 문맥어(상한/임계/목표)가 없거나 결과 서술어가 있으면 거부("커버리지 85%", "상한 20% 초과로 3/5 실패"; "오차 상한 ±20%" 는 경고), 수량+단위는 결과 서술어가 있고 계약 문맥어가 없을 때만 거부("실패 3건 발견"; "함수 50줄 이내", "파일 5개", "실패 상한 3회" 는 경고 — 경고 처분은 유효한 증거 블록이 하나 이상 있을 때이고 블록이 없으면 이전처럼 전부 거부), 거부는 리뷰 회차를 소모하지 않음; 주석 규칙이 지속되는 계약만 남기도록 바뀌어 회차·수리 이력·측정 결과는 `trail/` 로 보내고 리뷰어는 추가·수정된 주석 줄만 보며 박제된 서술은 정정 대신 삭제·이동; 문서·기록만 바뀐 리뷰 회차는 현재 관측된 모든 변경 경로가 리뷰 시작 시점 관측(`changeset_paths`)에 포함될 때 타입체크·테스트 증거 없이 진행(경로 집합 포함 관계 검사, 내용 비교 아님; `--print-subject` 가 그 관측을 `changeset_paths` 로 보고, 보안 검토 출력은 그대로). ([CHANGELOG](CHANGELOG.md))

이전: **v2.0.3** (2026-09-04) — 사용자 대면 결함 수정 4건: 프로젝트에 별도 설정이 없을 때 보안 검토 게이트가 조용히 사라지던 문제 해소(정책 동봉), git 저장소가 아닌 폴더의 온보딩 안내 통일(rein 은 git 저장소가 필요, `git init` 전에 확인, `--allow-non-git` 은 명시적 opt-in), rein 런타임 상태 파일을 `.gitignore` 에 등록해 리뷰 지문이 흔들리지 않음, 초기화하지 않은 프로젝트에서 세션을 끝내도 `trail/` 잔여물을 남기지 않음. ([CHANGELOG](CHANGELOG.md))
