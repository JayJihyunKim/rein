...
최신 릴리즈: **v1.6.0** (2026-07-10) — 코드 리뷰(`/codex-review`)가 최신 codex 프론티어 모델(gpt-5.6-sol, 판정 일관성을 위해 단일 고정)로 심사하고, 독립 의견(`/codex-ask`)은 질문 성격에 따라 빠른/균형/깊은 모델을 자동 선택합니다. 리뷰 통과 기록에 어떤 모델·강도로 심사했는지 남습니다. ([CHANGELOG](CHANGELOG.md))
