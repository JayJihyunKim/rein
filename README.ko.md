...
최신 릴리즈: **v1.3.6** (2026-05-23) — 버그 3건: 새 설계/플랜 첫 리뷰가 무관한 활성 DoD 를 끌어와 거짓 NEEDS-FIX 를 내던 문제를 닫고, `rein job stop` 이 멈춘 잡을 `running` 으로 두지 않고 종료 상태(`killed` + 종료 코드)를 기록하며, 프로젝트 위치 탐지가 직접 CLI 경로에서도 오염된 git 환경변수를 차단합니다. ([CHANGELOG](CHANGELOG.md))
