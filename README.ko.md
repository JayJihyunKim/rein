...
최신 릴리즈: **v1.4.6** (2026-06-08) — codex 모델명을 한 파일에서 관리. 이전에는 codex 리뷰·질문 스킬 여러 곳에 모델명이 하드코딩돼 codex 가 모델을 rename 하면 죽은 이름이 남아 리뷰가 조용히 실패했습니다. 이제 모델명은 한 파일(`plugins/rein-core/config/codex-models.sh`)에서 역할별(코드 리뷰·분석)로 관리하고, 모델이 거부되면 고쳐야 할 항목을 경로와 함께 짚어 안내하며 잘못된 모델로 리뷰가 통과되는 것도 막습니다. ([CHANGELOG](CHANGELOG.md))
