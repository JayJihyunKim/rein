...
최신 릴리즈: **v1.5.6** (2026-06-17) — 문서·운영기록·버전 문자열만 바꾼 커밋은 보안 재검토를 자동으로 건너뜁니다(같은 명령줄에 `git add`를 끼우거나 wrapper·백그라운드·다른 저장소 지정, 또는 허용 목록 밖 파일이 섞이면 예전처럼 보안 검토를 요구). v1.5.5의 보안 신선도 검사가 낳은 릴리스 마찰을 없애되, 허용 목록 밖은 전부 보안 요구(fail-closed)를 유지합니다. ([CHANGELOG](CHANGELOG.md))
