...
최신 릴리즈: **v1.3.3** (2026-05-20) — 매 turn / 매 Bash 호출마다 들어가던 rule 본문을 약 91~92% 줄였습니다 (answer-only-mode 7 KB / background-jobs 6 KB → short summary 각 ≤ 600 B). background-jobs advisory hook 은 hot-path 명령 (pytest / npm test / cargo build / playwright / make / tsc 등 — bare + args) 에만 spawn 합니다. 차단 가드 (safety-guard, test-commit-gate, bootstrap check) 는 always-on 그대로. ([CHANGELOG](CHANGELOG.md))
