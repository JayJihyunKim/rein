...
최신 릴리즈: **v1.6.3** (2026-07-23) — 페르소나가 기본 중립·명시 활성만으로 바뀌고, 내장 페르소나 `jennie` 와 `persona` 스킬(고르기 · 질문 몇 개로 커스텀 생성)이 추가되며, 페르소나를 바꾸는 순간 그 캐릭터의 인사말이 대화 언어에 맞춰 한 줄 뜨고, 코드 리뷰(`/codex-review`)에 멈춤 감지 시간 상한이 생겨 매달린 리뷰를 자동 감지·대체합니다. ([CHANGELOG](CHANGELOG.md))
