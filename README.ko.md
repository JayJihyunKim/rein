...
최신 릴리즈: **v1.4.3** (2026-06-02) — 스펙 작성 에이전트 + 계획 작성 진입점 통일 + 훅 속도·견고화. 새 `spec-writer` 에이전트가 brainstorm 문서를 rein 규격 스펙으로 작성하고 자동 검토를 거칩니다(계획 작성 흐름과 동일). `/writing-plans` 스킬을 제거해 계획 작성은 항상 `plan-writer`(및 그 검토 단계)를 거칩니다. 편집/Bash 훅이 Python 인터프리터를 덜 띄우고(POSIX 에서 실행-존재 확인 생략 — Windows 진단 경로는 그대로), 차단 메시지 생성기에 무결성 검증을 추가해 "설치됐지만 깨진 python3" 가 차단을 우회시키지 못하게 했습니다(항상 fail-closed + 명확한 진단). ([CHANGELOG](CHANGELOG.md))
