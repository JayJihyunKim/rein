...
최신 릴리즈: **v1.6.4** (2026-07-27) — 페르소나 목록에서 **새로 만들기**를 바로 고를 수 있고(이전엔 목록에 입구가 없어 도달 불가), 페르소나를 바꾸면 그 자리에서 즉시 적용되며, 만들 때 인사말을 직접 고르거나 쓸 수 있습니다. 코드 리뷰(`/codex-review`)가 테스트를 돌리는 중에 잘못 중단되던 문제도 고쳤습니다 — 진짜 멈춘 리뷰는 여전히 감지합니다. ([CHANGELOG](CHANGELOG.md))

이전: **v1.6.3** (2026-07-23) — 페르소나가 기본 중립·명시 활성만으로 바뀌고, 내장 페르소나 `jennie` 와 `persona` 스킬(고르기 · 질문 몇 개로 커스텀 생성)이 추가되며, 페르소나를 바꾸는 순간 그 캐릭터의 인사말이 대화 언어에 맞춰 한 줄 뜨고, 코드 리뷰(`/codex-review`)에 멈춤 감지 시간 상한이 생겨 매달린 리뷰를 자동 감지·대체합니다. ([CHANGELOG](CHANGELOG.md))
