...
최신 릴리즈: **v1.4.2** (2026-06-01) — 계획 기반 병렬 실행. 새 `parallel-execute` 스킬이 계획에 선언된 작업 의존 관계를 읽어, 서로 의존하지 않는 편집 전용 작업을 같은 작업 트리에서 동시 서브에이전트로 실행하고, 의존하거나 파일을 바꾸는 작업은 순서대로 실행합니다. 각 묶음이 끝날 때마다 메인 세션이 검증·테스트·커밋하며, 호스트가 병렬을 지원하지 않으면 순차로 내려갑니다. 이번 릴리스는 자율 진행 시 코드/보안 리뷰 게이트를 건너뛸 수 있던 틈도 막고, 파일 수정 시각만 바뀌는 브랜치 조작에서의 거짓 리뷰 차단을 줄였습니다. ([CHANGELOG](CHANGELOG.md))
