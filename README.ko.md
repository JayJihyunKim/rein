...
최신 릴리즈: **v1.3.4** (2026-05-22) — 보안 리뷰 stamp 면제를 명시적 active DoD(Tier 1)에서만 적용하도록 좁히고 (advisory mtime-fallback 은 더 이상 보안 리뷰 면제의 근거가 아님), safety guard 의 비밀파일 읽기 차단(P8)을 `grep` / `awk` / `sed` / `jq` / `cut` 으로 확장하고, 세션 종료 게이트를 강화했습니다 (stale resolver-cache 정리, 완료 DoD stale 경고 제외, POSIX 정수 검사). ([CHANGELOG](CHANGELOG.md))
