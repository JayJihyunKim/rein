...
최신 릴리즈: **v1.5.7** (2026-06-18) — 코드 리뷰(`/codex-review`)와 설계 리뷰가 어느 폴더에서 실행하든 설치된 플러그인의 도우미 스크립트를 정확히 찾습니다. 이전에는 안내 문서가 현재 폴더 기준 상대 경로를 써서, 그 폴더가 없는 사용자 저장소에서는 호출이 어긋날 수 있었습니다. ([CHANGELOG](CHANGELOG.md))
