...
최신 릴리즈: **v1.4.1** (2026-05-29) — 답변이 사용자 언어를 따라갑니다. rein 이 매 턴 주입하던 규칙 텍스트가 한국어라, 영어로 설정된 프로젝트에서 영어로 질문해도 답변이 한국어로 쏠릴 수 있었습니다 ("한국어로 답하라"는 강제는 없었고, 상시 주입되는 한국어 컨텍스트의 soft anchoring 이 원인). 이 텍스트를 영어로 바꾸고, "사용자 메시지의 언어로 답하되 Claude Code 의 명시적 `language` 설정이 있으면 우선"하는 출력 언어 정책을 추가했습니다. 언어를 고정해 둔 경우 영향 없음. 이번 릴리스에는 리뷰 게이트 정확도 수정(내용 변경 없는 브랜치 조작에서의 거짓 차단 해소, `tests/` 편집을 미리뷰 스펙 게이트에서 제외)과 커밋 가드 견고화도 함께 포함됩니다. ([CHANGELOG](CHANGELOG.md))
