...
최신 릴리즈: **v1.6.2** (2026-07-22) — 코드 리뷰(`/codex-review`)가 리뷰 시작 전에 로컬 검증 증거를 요구합니다: 변경이 있는 요청은 타입체크·테스트 통과 증거 블록과 자기 diff 검토 한 줄을 갖춰야 하며(검증 명령이 없는 프로젝트·의도된 TDD red 단계는 선언으로 통과), 없으면 리뷰 비용을 쓰기 전에 거부됩니다. 리뷰 응답도 짧아져 통과 항목은 섹션별 카운트 요약으로 줄고 지적 사항만 전량 상세 서술됩니다(`REIN_REVIEW_VERBOSE=1` 로 전체 서술 복원). ([CHANGELOG](CHANGELOG.md))
