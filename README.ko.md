...
최신 릴리즈: **v2.2.0** (2026-09-23) — 리뷰 게이트가 문서 지적의 차단 기준을 문서 봉투와 코드 봉투에 같은 문장으로 명시합니다: 결정·범위·사실의 오류만 차단합니다. 표현·배치·명칭 취향은 참고사항으로 분리되고, 잘못된 사용법·영향 범위 과장·필수 보안·안전 제약의 누락(인증·TLS 검증·안전한 역직렬화 등)·완료 주장·수치 불일치·추적 누락은 계속 차단되며, Design Alignment 의 "Scope ID 포맷" 검사는 ID 문자열이 아니라 Scope Items 표의 설명·검증 방법까지 읽어 판정합니다. 재리뷰 회차는 직전 보고에 High 가 없고 필수 검사 축이 수행됐으며 변경이 지적 처리에 한정되면(새 인터페이스·보안 경계·위험 경로·범위 변경 없음) `[EFFORT:medium]` 이 기본값이 되고, 직전 보고에 High 가 있었거나(그 High 를 고쳤더라도 그 회차는 대상 밖) 지적 처리를 넘는 변경이면 회차 숫자 대신 남은 위험 기준으로 `[EFFORT:high]` 로 올리며, 필수 축을 건너뛴 검토 뒤의 회차는 마커 없이 산출 강도(위험 경로 하한 포함)로 전체 검토합니다. 리뷰 소요 시간 기록에 `effort`·`effort_source` 두 필드가 추가돼 실제 사용한 강도와 그 출처가 남습니다. "메인 세션 지휘" 규칙을 켠 저장소(또는 `rein:orchestrator` 를 명시적으로 부른 경우)에서는 보안 리뷰 통과 기록을 부모 세션이 직접 발급합니다 — 검토 시작 시점의 변경 지문을 잡고, `rein:security-reviewer` 에 검토를 맡기고, 반환 블록을 대조한 뒤 발급하며 계약 위반이면 발급을 거부합니다. 단독 호출은 종전처럼 검토자가 직접 발급합니다. ([CHANGELOG](CHANGELOG.md))

이전: **v2.1.3** (2026-09-22) — v2.1.2 의 후속 패치: 치환 문자열의 백슬래시를 접지 않는 awk(macOS 기본 awk, BusyBox awk)에서 리뷰 소요 시간 기록이 값 안의 백슬래시를 두 번 이스케이프하던 문제를 문자열 연결 방식으로 고치고, 테스트 이식성을 손봤습니다. v2.1.2 의 내용 — 저장소 단위로 켜는 "메인 세션 지휘" 규칙, 명시적 호출 옵션 경로가 된 `rein:orchestrator`, 워커 실행용 Builder 완료 기준, 리뷰 소요 시간 기록, 후속 요청서 양식 — 이 그대로 들어 있습니다. ([CHANGELOG](CHANGELOG.md))
