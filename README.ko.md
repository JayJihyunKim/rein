...
최신 릴리즈: **v1.5.5** (2026-06-16) — 리뷰·보안 검토가 오래되거나 통과가 아닌 기록으로 조용히 통과되던 구멍을 봉합합니다. 이제 커밋하려면 보안 검토가 코드 리뷰보다 같거나 새 것이고(같은 작업 사이클) 코드 리뷰가 실제 "통과"여야 하며, 설계 리뷰 대기 표시 생성기가 실패하면 조용히 통과하는 대신 차단(fail-closed)하고, 설계 리뷰 1회 우회 표시는 한 번 쓰면 소비됩니다. 또 커버리지 검증기가 번호 붙은 제목과 백틱으로 감싼 ID를 그대로 인식해 설계 문서를 손으로 형식 맞출 필요가 줄었습니다. ([CHANGELOG](CHANGELOG.md))
