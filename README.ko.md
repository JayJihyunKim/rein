...
최신 릴리즈: **v2.1.1** (2026-09-08) — 사용자 대면 결함 수정 3건: 같은 작업 트리에서 일하는 서브에이전트는 변경성 `git stash` 를 더 이상 실행할 수 없음(훅 입력에 서브에이전트 표식이 있을 때만 Bash 가드가 차단, `stash list`/`show` 와 메인 세션은 무영향); 가드의 절 시작 인식이 `if … then`, `while … do`, `case … )`, 중괄호 그룹, 백틱 치환 안의 호출까지 분류기를 공유하는 네 규칙(`.env` 읽기·`.env` 커밋·파괴적 git 확인·서브에이전트 stash)에서 잡음 — 그런 토큰 바로 뒤의 단순 언급은 보수 방향으로 차단될 수 있으니 문장을 바꿔 다시 실행; 초기화 스크립트가 비 UTF-8 `.gitignore` 의 바이트를 보존하고 `.rein/project.json` 자리의 디렉터리를 거부. ([CHANGELOG](CHANGELOG.md))
