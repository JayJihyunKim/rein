...
최신 릴리즈: **v1.5.4** (2026-06-15) — 자동모드 안내를 실제 동작과 일치하도록 정정하고(자동모드를 켜도 반복 문제 관련 종료 차단만 조용해지며, 작업 기록 확인 차단은 그대로 발동 — 자율 작업 중에도 세션 종료 전 trail 기록은 갱신), 파일을 편집할 때마다 뜨던 "변경 파일 수가 작업 정의와 안 맞음" 거짓 경고를 제거합니다. 오래 방치된 버전 관리 밖 파일이 매번 변경 목록에 섞이던 것을, 이제 이번 세션에 실제로 새로 만들어진 파일만 대상으로 봅니다. ([CHANGELOG](CHANGELOG.md))
