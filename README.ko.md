...
최신 릴리즈: **v1.3.5** (2026-05-22) — 리뷰 게이트 무결성 갭 3건을 닫았습니다: 리뷰가 끝난 설계/플랜을 다시 편집하면 재리뷰를 강제하고(남아있던 `.reviewed` 가 더 이상 소스 편집을 풀지 못함), 활성 DoD 마커를 작업 선택 단계에서도 경로 검증하여(세션 시작 청소뿐 아니라) 오염된 마커가 프로젝트 밖을 가리키지 못하게 하며, `/codex-review` 가 스탬프에 기록된 `diff_base` 가 HEAD 에서 도달 가능한 실재 커밋인지 검증합니다(아니면 HEAD~1 로 대체). ([CHANGELOG](CHANGELOG.md))
