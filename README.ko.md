...
최신 릴리즈: **v1.6.6** (2026-08-07) — 차단된 명령의 원문이 더 이상 git 추적 incident 로그에 남지 않습니다: 추적 기록에는 명령 종류와 내용 지문(해시)만 남고, 마스킹 적용본(환경변수형 토큰·Authorization 헤더·URL 자격증명)은 git 밖 로컬 로그에만 저장됩니다. 테스트 하니스 이벤트는 출처 표기되어 반복 위반 경고·incident 승격에서 제외됩니다. ([CHANGELOG](CHANGELOG.md))
