...
최신 릴리즈: **v2.1.3** (2026-09-22) — v2.1.2 의 후속 패치: 발행 전 macOS 자체 검사에서 awk 이식성 결함이 드러나 v2.1.2 의 마켓플레이스 발행이 막혔습니다 — 치환 문자열의 백슬래시를 접지 않는 awk(macOS 기본 awk, BusyBox awk)에서는 새 리뷰 소요 시간 기록이 값 안의 백슬래시를 두 번 이스케이프했습니다(JSON 구조는 유지, gawk·mawk 를 쓰는 일반적인 리눅스·WSL2 환경은 영향 없음). v2.1.2 의 내용은 이 버전에 그대로 들어 있습니다 — 저장소 단위로 켜는 "메인 세션 지휘" 규칙: 프로젝트의 `.rein/policy/rules.yaml` 에 `orchestrator-first:` 아래 `enabled: true` 를 넣으면, 다음 세션부터 세션 시작 안내가 메인 세션에게 비trivial 개발 작업(구현·버그수정·리팩토링)을 나누고, 독립된 단위를 워커 서브에이전트에게 병렬로 맡기고, 리뷰를 워커마다가 아니라 웨이브마다 한 번 하도록 알려 줍니다. 기본은 꺼짐 — 항목을 넣지 않으면 이 규칙 요약은 세션 시작 안내에 추가되지 않고, 정책 파일이 깨졌거나 읽을 수 없어도 켜지지 않습니다. 이 설정과 무관하게, `rein:orchestrator` 에이전트는 기본 진입점이 아니라 명시적으로 부를 때 쓰는 옵션 경로로 설명이 바뀌고, Builder 에이전트 세 개의 완료 기준은 단독 실행과 워커 실행(부모가 웨이브 끝에서 코드 리뷰와, 면제되지 않은 경우 보안 리뷰를 수행)을 구분합니다. 함께 들어간 것: `/codex-review` 호출이 판정으로 끝날 때마다 `trail/review-events/` 아래에 시각·리뷰 종류·판정·회차·소요 시간을 한 줄 남기고(통과해도 지워지지 않음), 래퍼가 재리뷰마다 새 검토 세션을 띄우는 점에 맞춰 스킬 안내에 2회차 이후용 후속 요청서 양식이 추가됩니다. ([CHANGELOG](CHANGELOG.md))

이전: **v2.1.0** (2026-09-07) — 사용자 대면 변화 3건: 리뷰 요청서의 증거 블록 밖에 실행 결과 서술이 남아 있으면 codex 호출 전에 거부(걸린 줄과 고쳐 쓰기 안내, 회차 비소모); 주석 규칙이 지속되는 계약만 남김; 문서·기록만 바뀐 리뷰 회차는 변경 경로가 모두 리뷰 시작 시점 관측(`changeset_paths`)에 포함되면 타입체크·테스트 증거 없이 진행. ([CHANGELOG](CHANGELOG.md))
