...
## v2.2.0 — 2026-09-23 (리뷰 게이트의 문서 지적 차단 기준·재리뷰 강도 계약 + 보안 통과 기록의 발급 주체 이관)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **`/codex-review` 가 문서 지적을 차단하는 기준이 두 봉투(문서 리뷰·코드 리뷰)에 같은 문장으로 명시됩니다.** 설계·계획·안내·CHANGELOG·README 에 대한 지적이 "수정 필요" 가 되려면 결정·범위·사실의 오류여야 하고, 같은 뜻의 표현 선택·배치·명칭 취향은 참고사항으로 분리돼 판정을 올리지 않습니다. 실질 결함 — 잘못된 사용법, 영향 범위 과장, 필수 보안·안전 제약의 누락(인증·TLS 검증·안전한 역직렬화 등), 완료 주장, 수치 불일치, 누락된 추적 연결 — 은 계속 차단합니다. Design Alignment 의 "Scope ID 포맷 미달" 도 문자열 형식이 아니라 Scope Items 표의 설명·검증 방법까지 읽어 판정합니다.
- **재리뷰 강도가 조건부 기본값이 됩니다.** 직전 독립 검토 보고에 High 가 없고 필수 검사 축이 수행됐으며, 변경이 직전 지적 처리에 한정되고(새 인터페이스·보안 경계·범위 변경 없음 — 보안 경계는 보안 규칙이 다루는 범주 전부이며, 위험 경로(hooks·security·config·workflows·`scripts/rein-*.sh`) 변경도 같은 제외 조건), 새 판단이 없으면 후속 회차는 `[EFFORT:medium]` 으로 부릅니다. 직전 보고에 High 가 있었으면(그 High 를 고쳤더라도 그 회차는 medium 대상 밖입니다) 또는 "지적 처리에 한정·새 판단 없음" 조건에 어긋나면 회차 숫자가 아니라 남은 위험 기준으로 `[EFFORT:high]` 로 올리고, 직전 검토가 필수 검사 축을 수행하지 않았으면 그 검토는 완결이 아니므로 마커 없이 산출 강도(위험 경로 하한 포함)로 전체 검토합니다. 이 기본값은 medium 이 놓치는 결함 폭을 재지 않은 운영 가설이며, 아래 기록 필드로 관측합니다.
- **리뷰 소요 시간 기록(`trail/review-events/`)에 강도 필드 두 개가 추가됩니다** — `effort`(실제 전달한 강도)와 `effort_source`(마커 / 산출 / 산출+위험 경로 하한 / fail-closed). 기존 8개 필드 뒤에 붙고, 값이 없으면 `null` 입니다.
- **메인 세션 지휘 규칙을 켠 저장소(또는 `rein:orchestrator` 를 명시적으로 부른 경우)에서는 보안 통과 기록을 지휘자(부모 세션)가 발급합니다.** 부모가 검토 시작 시점의 변경 지문을 잡아 `rein:security-reviewer` 에 검토를 맡기고, 반환 블록을 대조한 뒤 그 지문으로 발급합니다 — 대조 계약 위반이면 발급을 거부합니다(fail-closed). 단독으로 `rein:security-reviewer` 를 부르면 종전처럼 검토자가 직접 발급합니다(이중 모드). 부모의 dispatch 신호가 하나라도 있으면 검토자는 워커 모드로 들어가고, 그 안에 유효한 검토 범위가 없으면 리뷰하지 않고 차단으로 되돌립니다 — 검토자가 직접 발급하는 일은 없습니다. 관련 문서: `rein:orchestrator`·`rein:security-reviewer` 에이전트 정의, 운영 시퀀스·지휘 규칙, 병렬 실행 스킬, AGENTS.md.

## v2.1.3 — 2026-09-22 (v2.1.2 의 후속 patch — 일부 awk(macOS 기본 awk 등)에서 리뷰 소요 시간 기록의 백슬래시 이중 이스케이프)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **일부 awk 에서 리뷰 소요 시간 기록의 값이 원문과 달라지던 문제를 고쳤습니다.** v2.1.2 에서 추가된 `trail/review-events/` 기록이, 치환 문자열 안의 백슬래시를 접지 않는 awk(macOS 기본 awk, BusyBox awk 등)에서는 값 안의 백슬래시를 두 번 이스케이프했습니다(작업 이름이나 경로에 `\` 가 든 경우). JSON 구조는 깨지지 않았지만 읽어 온 값이 원문과 달랐습니다. gawk·mawk 를 쓰는 일반적인 리눅스·WSL2 환경은 영향이 없었습니다.
- v2.1.2 는 이 문제가 발행 전 자체 검사의 macOS 테스트에서 드러나 마켓플레이스에 올라가지 않았습니다. v2.1.2 의 변경 사항은 이 버전에 그대로 들어 있습니다(아래 항목 참조).

## v2.1.2 — 2026-09-21 (저장소 단위로 켜는 "메인 세션 지휘" 규칙 + 리뷰 소요 시간 기록 + 재리뷰 요청서 양식)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **저장소 단위로 켜는 "메인 세션 지휘" 규칙이 추가됩니다 (기본 꺼짐).** 프로젝트의 `.rein/policy/rules.yaml` 에 `orchestrator-first:` 아래 `enabled: true` 를 넣으면, 다음 세션부터 세션 시작 안내에 지휘 규칙 요약이 함께 들어옵니다. 내용은 이렇습니다 — 비trivial 개발 작업(구현·버그수정·리팩토링)에서 메인 세션이 직접 나눌 수 있는지 판단하고, 서로 독립된 단위를 워커 서브에이전트에게 병렬로 맡기고, 결과를 모아 검증합니다. 구현은 워커가 하고, 리뷰는 워커마다가 아니라 동시에 실행한 묶음(웨이브)이 끝날 때마다 부모가 한 번 합니다. 워커 모델을 바꾸거나 병렬을 포기할 때는 평문 한 줄로 알립니다. 조사·문서·설계 작업은 대상이 아닙니다. 항목을 넣지 않으면 이 규칙 요약은 세션 시작 안내에 추가되지 않고, 정책 파일이 깨졌거나 읽을 수 없어도 켜지지 않습니다(안전한 쪽으로 실패). 아래 항목들은 이 설정과 무관하게 적용됩니다. 전체 본문은 플러그인의 `rules/orchestrator-first.md` 에 있습니다.
- **`rein:orchestrator` 에이전트의 설명이 "명시적으로 부를 때만 쓰는 옵션 경로" 로 바뀝니다.** 이전에는 비trivial 개발 작업의 기본 진입점으로 소개됐습니다. 오케스트레이션 전체를 별도 컨텍스트로 격리하고 싶을 때 직접 호출하는 용도로 남고, 에이전트의 계약(워커 금지목록·깊이 규칙·웨이브 끝 통합 절차·워커 매핑)은 그대로입니다.
- **Builder 에이전트 세 개(`feature-builder` / `feature-builder-fix` / `feature-builder-refactor`)의 완료 기준이 단독 실행과 워커 실행을 구분합니다.** 부모가 작업 단위·쓰기 범위·금지목록을 넘겨 워커로 실행한 경우, 코드 리뷰·수정 후 재리뷰와 (면제되지 않은 경우의) 보안 리뷰는 부모가 웨이브 끝에서 수행합니다 — 새로 면제되는 것은 없고 수행 주체만 부모로 옮겨지며, 커밋 시점의 리뷰 요구는 그대로입니다. 그런 전달 없이 단일 하위 에이전트로 부르면 이전처럼 에이전트가 직접 리뷰합니다.
- **리뷰가 판정으로 끝날 때마다 소요 시간이 한 줄씩 기록됩니다.** `/codex-review` 호출이 통과·수정 필요·반려 중 하나로 끝나면 `trail/review-events/` 아래 파일에 시각·리뷰 종류·판정·호출 시점 회차·소요 시간 등을 한 줄 남깁니다. 기존 회차 카운터와 달리 통과해도 지워지지 않아 리뷰에 시간이 얼마나 드는지 나중에 볼 수 있습니다. 요청서 거절·시간 초과·회차 예산 소진처럼 판정 없이 끝난 호출은 기록하지 않으므로 전체 대기 시간의 합계로 쓰면 안 됩니다. 기록에 실패해도 리뷰 결과와 종료 코드는 바뀌지 않습니다.
- **`/codex-review` 안내에 재리뷰용 "후속 회차 요청서 양식" 이 추가됩니다.** 리뷰 래퍼는 재리뷰마다 새 세션을 띄우므로 이전 회차의 내용이 자동으로 이어지지 않습니다. 그래서 2회차부터는 요청서에 다섯 가지 — 직전 판정, 열린 지적을 어떻게 처리했는지(고침·위험 수용·반박), 이미 끝난 지적, 직전 이후 바뀐 것, 영향받는 계약과 증거 — 를 직접 적도록 안내합니다. 이전 안내에 있던 "같은 사이클에서는 세션을 이어간다" 는 설명은 실제 동작과 달라 바로잡았습니다.
- 작업 기록(DoD)의 `## 라우팅 추천` 에 선택 필드 두 개가 생깁니다. `orchestration:` 은 작업을 나눠 워커에게 병렬로 맡기는 방식을 쓰겠다는 승인을 `agent:` 와 별도로 받을 때 쓰고, `worker_strategy:` 는 주로 어떤 워커를 쓸지 적어 두는 참고 칸입니다. 둘 다 쓰지 않아도 됩니다.

## v2.1.1 — 2026-09-08 (서브에이전트 stash 차단 + Bash 가드 절 시작 인식 확대 + 초기화 스크립트 수리)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **같은 작업 트리에서 일하는 서브에이전트는 더 이상 `git stash` 를 할 수 없습니다.** 병렬 워커가 stash 를 왕복하면 부모 세션이 편집 중이던 파일이 통째로 사라졌다 되돌아와 충돌 마커를 남기던 결함의 구조적 수리입니다. Bash 안전 가드가 훅 입력에 서브에이전트 표식(`agent_id`)이 있을 때만 변경성 stash(인자 없음·push/pop/apply/drop/clear/save/branch/create/store/export/import·옵션으로 시작하는 형태, `git -C <dir> stash` 같은 전역 옵션 형태 포함)를 JSON deny 로 막고, 작업 트리를 직접 편집하거나 부모에게 blocked 를 보고하라고 안내합니다. `git stash list`/`show` 와 메인 세션은 영향이 없습니다. 병렬 실행 스킬의 워커 금지 목록에도 같은 항목이 추가됐습니다.
- **Bash 안전 가드가 제어문 안의 명령도 실제 호출로 봅니다.** 이전에는 `if …; then git reset --hard; fi` 처럼 예약어·중괄호 그룹·백틱 치환·`case` 패턴 뒤에 오는 호출을 "언급"으로 분류해 파괴적 git 명령 확인·`.env` 커밋 방지·stash 차단이 모두 건너뛰었습니다. 이제 `; & | ( ) {`·백틱·`if then elif else while until do !` 뒤가 절 시작으로 인식됩니다(분류기를 공유하는 네 규칙 — `.env` 읽기 차단·`.env` 커밋 방지·파괴적 git 명령 확인·서브에이전트 stash 차단 — 에 적용되고, 파이프-셸 실행 차단과 `.env` 스테이징 차단은 별도 판정이라 그대로입니다). 반대로 그런 토큰 바로 뒤에 놓인 단순 언급(`echo then git …`)은 보수 방향으로 차단될 수 있습니다 — 가드는 토큰 형태만 보고 문법 위치는 판단하지 않습니다(따옴표를 인식하지 않는 기존 한계와 같은 클래스). 차단되면 문장을 바꿔 다시 실행하면 됩니다.
- **초기화 스크립트가 비 UTF-8 `.gitignore` 에서 멈추지 않습니다.** Latin-1 주석 등이 든 `.gitignore` 를 읽다 traceback 으로 초기화가 실패하던 문제를, 기존 바이트를 그대로 보존하고 rein 패턴만 덧붙이는 방식으로 고쳤습니다. `.rein/project.json` 자리에 디렉터리가 있으면 traceback 대신 원인을 말하며 거부하고, 무시 목록 보강 단계는 그런 프로젝트를 "초기화됨"으로 오인하지 않습니다.

## v2.1.0 — 2026-09-07 (리뷰 요청서 거부 규칙 + 주석 규칙 개정 + 문서-only 리뷰 조기 통과)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **리뷰 요청서에 증거 없는 실행 결과 서술이 남아 있으면 `/codex-review` 가 codex 를 부르기 전에 거부합니다.** 이전에는 증거 블록이 하나라도 있으면 블록 밖 수치·통과 주장은 경고만 하고 넘어갔습니다. 이제 블록 밖 문장은 세 형태로 판정합니다 — (1) "테스트 통과" 처럼 검증 대상과 통과 표현이 함께 있는 문장은 항상 거부, (2) 비율·백분율은 상한·임계·목표 같은 계약 문맥어가 **없거나** 발견·실패 같은 결과 서술어가 **있으면** 거부(예: "커버리지 85%", "상한 20% 초과로 3/5 실패" — 계약 문맥어만 있는 "오차 상한 ±20%" 는 경고), (3) 수량+단위는 결과 서술어가 있고 계약 문맥어가 **없을 때만** 거부(예: "실패 3건 발견" — "함수 50줄 이내", "파일 5개", 둘 다 있는 "실패 상한 3회" 는 경고). 이 경고 처분은 유효한 증거 블록이 **하나 이상 있을 때**의 이야기이고, 블록이 하나도 없으면 이전처럼 모든 정량·통과 서술이 거부됩니다. 거부 시 종료 코드 4 로 어느 줄이 걸렸는지와 고쳐 쓰는 방법(증거 블록으로 옮기거나 계약 형태로 다시 쓰기)을 안내하며, codex 호출 전이라 리뷰 회차를 소모하지 않습니다.
- **주석 규칙이 "지속되는 계약만" 으로 바뀝니다.** 코드 주석에는 설계 근거·불변식·고정 임계값만 남기고, 회차 서수("3차 수정")·수리 이력("이전에는 X 였으나")·실행 결과 수치("N/M 통과")는 작업 기록(`trail/`)으로 보냅니다. 리뷰어는 diff 에서 추가되거나 수정된 주석 줄만 보며 기존 주석 전체의 일괄 청소를 요구하지 않고, 이미 박제된 서술은 현재값으로 정정하는 대신 삭제하거나 이동하라고 안내합니다(세션 시작 규칙 요약과 대체 리뷰어 체크리스트에도 반영).
- **문서·기록만 바뀐 리뷰 회차는 타입체크·테스트 증거 없이 진행됩니다.** 리뷰 대상이 문서·`trail/` 뿐이면 자가검증 관문이 증거 블록을 요구하지 않습니다. 단 안전하게만 — 변경 파일 목록을 얻지 못했거나, 현재 관측된 변경 경로 중 리뷰 시작 시점 관측(`changeset_paths`)에 없던 경로가 하나라도 있으면(예: 그 사이 코드 파일이 생김) 이전처럼 증거를 요구합니다(검사는 경로 집합의 포함 관계이지 내용 비교가 아닙니다). 이를 위해 `rein issue-evidence code_review --print-subject` 의 JSON 에 `changeset_paths`(같은 관측의 전체 변경 경로) 키가 추가됐습니다. 보안 검토 축의 출력은 그대로입니다.

## v2.0.3 — 2026-09-04 (보안 검토 축 복구 + git 필수 온보딩 안내 통일 + 런타임 상태 파일 무시 규칙)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **보안 검토가 조용히 사라지던 문제가 해소됩니다.** 보안 검토 축의 정책이 배포본에 함께 담기지 않아, 프로젝트에 별도 설정이 없는 저장소에서는 보안 검토 기록이 남지 않은 채 커밋이 통과했습니다. 이제 정책이 배포본에 동봉되어, 프로젝트에 설정이 없으면 배포 기본값으로 동작합니다. 프로젝트 설정이 있는데 손상된 경우와 두 곳 모두 없는 경우에는 조용히 통과시키지 않고 안전하게 차단합니다. 보안 표면(인증·시크릿·설정 등)을 건드린 커밋은 보안 검토 기록이 있어야 통과합니다.
- **git 저장소가 아닌 폴더에서 세션 시작·프롬프트 안내·초기화 스크립트가 같은 말을 합니다.** 이전에는 세 채널이 서로 모순된 지시(초기화 거부 / 스크립트 실행 권유 / 정상 완료)를 내서 온보딩 상태를 판단할 수 없었습니다. 이제 rein 은 **git 저장소를 필요**로 하며, 에이전트가 먼저 물어본 뒤 승인하면 `git init` 과 초기화를 진행하고, 거부하면 그 폴더에서는 꺼진 채로 둡니다. 초기화 스크립트는 non-git 폴더를 기본 거부하고(`--allow-non-git` 으로만 opt-in), 초기화가 끝나면 같은 세션에서 바로 감시가 켜집니다(재시작 불필요). 안내에 나오는 복구 명령은 공백·특수문자가 든 경로에서도 그대로 붙여 넣어 실행되며 명령 검사 게이트를 통과합니다.
- **rein 런타임 상태 파일이 git 에 섞여 들어가 리뷰 기록이 거부되던 문제가 해소됩니다.** 초기화가 만드는 무시 목록에 `.rein/state.json` 등 매 도구 호출마다 바뀌는 런타임 파일이 빠져 있어, 초기 커밋에 추적된 뒤 리뷰와 커밋 사이에 검토 지문이 어긋났습니다. 이제 무시 목록에 포함되고, 이미 초기화된 프로젝트도 다음 세션 시작 때 무시 목록이 보강됩니다. 이미 추적 중인 저장소는 `git rm --cached .rein/state.json` 으로 한 번 추적을 해제하면 되고, 그 추적 해제 커밋도 이제 검토 지문을 흔들지 않습니다.
- **초기화하지 않은 프로젝트에서 세션을 끝내도 rein 이 기록 폴더 찌꺼기를 남기지 않습니다.** 이전에는 세션 종료 훅이 초기화 전 프로젝트에 `trail/` 잔여물을 만들어, 다음 세션이 그 프로젝트를 "초기화가 절반만 된 상태"로 오진했습니다. 이제 초기화가 끝난 프로젝트에서만 종료 기록을 남기고, 세션 종료 안내도 현재 폴더가 아니라 프로젝트 폴더의 기록을 읽습니다.
- 첫 세션 안내(프라이머) 끝에 초기화 상태 한 줄이 붙습니다.

> 버전 등급 메모: 사용자 대면 결함 수정 4건 묶음 — patch (2.0.3), 사용자 결정(2026-09-04). 주의: 초기화 스크립트가 git 저장소가 아닌 폴더에서 기본 거부(종료 코드 3)로 바뀐 것은 versioning 규칙 A 의 문자 그대로는 breaking 후보다. 그럼에도 patch 로 둔 사유 — 기존 git 저장소 사용자에겐 동작 변화가 없고, non-git 폴더는 원래 지원 범위 밖이었으며 `--allow-non-git` 한 플래그로 이전 동작을 되살릴 수 있다. 이 예외는 advisory 규칙에 대한 명시적 사용자 결정으로 기록한다. 메인테이너 테스트·내부 문서 변경은 사용자 동작 무관이라 본 목록에서 제외.
