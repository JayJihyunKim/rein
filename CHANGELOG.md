...
## v2.0.2 — 2026-09-01 (페르소나 선택 UI 개선 + 표시 이름 현지화)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **"페르소나 골라줘" 선택창이 세 개의 고정 메뉴로 단순해집니다.** 프리셋이 늘어나도 선택 흐름이 복잡해지지 않도록, 맨 위에서 `페르소나 목록` / `내 페르소나 만들기` / `페르소나 끄기` 중 하나를 먼저 고릅니다. `페르소나 목록` 을 고르면 프리셋이 표시 이름·요약과 함께 한눈에 나열됩니다.
- **페르소나가 사람이 읽는 이름으로 불립니다.** 이전에는 대화·선택 목록·세션 안내에서 내부 파일 이름(영어 slug)이 그대로 노출됐는데, 이제 표시 이름(마르코 / 제니 / 최행배)으로 나옵니다. 영어 표기를 가진 프리셋(마르코·제니)은 영어로 대화하면 그 표기를 쓰고, 표시 이름이 없는 예전 커스텀 페르소나는 안전한 대체 이름으로 표시됩니다.

> 버전 등급 메모: 사용자 대면 UI 개선 — patch (2.0.2). 메인테이너 테스트·설계 문서는 사용자 동작 무관이라 본 목록에서 제외.

## v2.0.1 — 2026-08-28 (hotfix for v2.0.0 — 업데이트 후 전체 편집 차단 해소)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **v2.0.0 으로 업데이트한 뒤 모든 파일 편집이 막히던 문제가 해소됩니다.** v2.0.0 은 "이 편집이 정의된 작업에 속하는지" 검사하는 게이트를 기본으로 켜두었는데, 그 게이트가 참조하는 정책 파일이 배포본에 함께 담기지 않았습니다 — 그 결과 기존 설정이 없는 프로젝트에서는 Edit/Write/MultiEdit 이 전부 복구 불가로 차단됐습니다(1.6.6 롤백 외 우회 불가). 이제 이 정책이 플러그인에 동봉되고, 프로젝트별 설정이 없으면 배포 기본값으로 정상 동작합니다. 활성 작업이 있으면 편집이 허용되고, 없으면 예전처럼 "작업 정의가 필요하다"는 복구 가능한 안내로 돌아갑니다.
- **정책을 지워 게이트를 무력화하기가 더 어려워집니다.** 프로젝트 설정 정책을 지워도 배포 기본값이 이어받아 게이트가 계속 동작하며, 두 곳 모두에 정책이 없는(=설치가 손상된) 경우에만 안전하게 차단됩니다. 그 경우 안내문이 실제 복구 명령(`claude plugin update rein`)을 알려줍니다.

> 버전 등급 메모: v2.0.0 의 설치 직후 전체 편집 차단(critical)을 푸는 hotfix — patch (2.0.1). 같은 날 hotfix 예외(critical 설치 장애). 메인테이너 테스트·내부 문서 변경은 사용자 동작 무관이라 본 목록에서 제외.

## v2.0.0 — 2026-08-27 (v2 governance — 통과 증거를 실제 검토에 결속)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **리뷰·보안 통과 증거가 실제 검토 결과에 묶입니다.** 이전에는 게이트가 "통과했다는 표식 파일" 의 존재만 확인했는데, 이제 통과 증거가 그 시점의 변경 내용 지문에 결속됩니다 — 검토를 안 거치고 표식만 만들거나, 검토 후 다른 내용으로 바꿔치는 우회가 차단됩니다. 커밋 게이트는 이 증거만으로 통과 여부를 판정합니다.
- **게이트가 자기 진술이 아니라 신뢰 저장소에서 권위를 얻습니다.** 리뷰·보안·활성 작업 판정이 에이전트/스크립트의 자기 주장 대신 별도 신뢰 저장소를 근거로 삼아, 상태를 위조해 게이트를 넘기기 어려워집니다.
- **설치·초기화 시 rein 런타임 폴더가 자동으로 git 무시 목록에 등록됩니다.** 이전에는 증거 기록 폴더(`.rein/state/`) 등이 추적되지 않은 채 검사 대상에 섞여, 설치 직후 첫 리뷰 기록이 자기 자신을 무효화해 커밋이 막힐 수 있었습니다. 이제 설치 시 무시 규칙을 안전하게 등록합니다 (심볼릭 링크 등 위험 케이스는 안전하게 실패).
- **부산 사투리 승부사 "최행배" 페르소나가 기본 프리셋에 추가됩니다.** "페르소나 골라줘" 에서 boss-ace·jennie 와 함께 고를 수 있습니다.

> 버전 등급 메모: 게이트 동작이 통째로 재구성되는 user-facing breaking — major (2.0.0). 사용자 세션 동작과 무관한 내부 변경은 본 목록에서 제외.

## v1.6.6 — 2026-08-07 (차단 로그 민감정보 봉합)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **차단된 명령의 원문이 더 이상 git 추적 기록에 남지 않습니다.** 이전에는 안전 가드가 명령을 차단하면 명령 원문 전체가 저장소에 추적되는 incident 로그에 그대로 기록됐습니다 — 명령에 토큰·비밀번호가 인라인으로 있으면 차단한 가드가 오히려 비밀값을 저장소 히스토리에 보존하는 구조였습니다. 이제 추적 기록에는 명령 종류와 내용 지문(해시)만 남고, 디버깅용 원문은 git 밖 로컬 로그(`.rein/logs/`, 자동 회전)에 마스킹 적용본으로만 저장됩니다. 마스킹은 `GITHUB_TOKEN=` 류 환경변수형, Authorization 헤더, URL 자격증명, basic auth 값을 가립니다.
- **반복 위반 경고와 incident 승격이 테스트 소음에 오염되지 않습니다.** 테스트 하니스가 발생시킨 차단 이벤트는 출처 표기되어 반복 경고 카운트·incident 집계·세션 종료 안내에서 제외됩니다. 과거에 기록된 무표기 이벤트는 실사용으로 취급합니다(경고 누락 방지).

> 버전 등급 메모: user-facing 버그/보안 수정 — patch (1.6.6). 메인테이너 인프라 변경(전 스위트 자동 CI, 배포 전 검증 연결)은 사용자 동작 무관이라 본 목록에서 제외.

## v1.6.5 — 2026-08-05 (리뷰 회차 상한·문서 리뷰 전용 판정 + 게이트 오차단 3건 수리)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **코드 리뷰(`/codex-review`)에 회차 상한이 생깁니다 (기본 5회).** 같은 작업에서 미통과 재리뷰가 상한에 도달하면 리뷰를 실행하지 않고 거부하며, 지금까지의 지적을 정리해 사람에게 넘기라고 안내합니다(전용 exit 6). 더 돌리려면 요청서 첫 줄에 `[MAX_ROUNDS:숫자]` 를 단독으로 선언해야 하고, 연장 사실은 경고와 기록으로 남습니다 — 조용히 늘어나는 경로는 없습니다. 통과하면 카운터는 정리됩니다. 배경: 상한이 문서 지시로만 있던 동안 13회 무보고 연속 리뷰가 실측됐습니다.
- **설계·계획 문서 리뷰가 문서에 맞는 기준으로 진행됩니다.** 이전에는 문서 리뷰에도 코드 리뷰 지시(자원 누수·테스트 대조 찾기)가 그대로 나가, 계획서에 구현 세부를 쌓게 만드는 지적이 반복됐습니다. 이제 문서 리뷰는 결정 근거·범위·추적성 축으로만 판정하고, 구현 수준 지적은 참고사항으로 분리됩니다. 문서 리뷰 프롬프트가 절반으로 줄어듭니다.
- **다른 저장소 문서의 리뷰 표식이 이 저장소 편집을 잠그지 않습니다.** 표식의 대상 경로를 실제(물리) 경로 기준으로 판정해, 저장소 밖을 가리키는 표식은 경고만 하고 무시합니다.
- **미리뷰 설계 문서가 있어도 무관한 파일 편집은 막히지 않습니다.** 지금 진행 중인 작업이 참조하는 문서의 미리뷰만 차단하고, 무관한 문서는 경고로 알려줍니다 (설계→코딩 순서 강제는 유지). 이전에는 문서 하나가 미리뷰면 소스 전체가 잠겼습니다 (한 세션 28회 실측).
- **스크립트 파일에 기록될 텍스트 속 `git commit` 줄이 실제 커밋으로 오인되지 않습니다.** 또한 임시 폴더에 리터럴 경로로 이동해 실행하는 샌드박스 커밋은 커밋 게이트를 건너뜁니다 — 단 이동 대상이 변수이거나, 이 저장소(연결형 워크트리 포함)를 가리키면 종전대로 차단됩니다.

> 버전 등급 메모: Rule A 기계 판정은 minor(리뷰 회차 상한·문서 리뷰 판정 등 새 동작). **사용자 결정으로 patch(1.6.5) 릴리스** — 결정 주체와 기계 판정을 모두 기록해 둔다.

## v1.6.4 — 2026-07-27 (페르소나 만들기 입구·전환 즉시 반영 + 코드 리뷰 멈춤 오판 수리)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **페르소나 목록에서 바로 "새로 만들기" 를 고를 수 있습니다.** 이전에는 커스텀 페르소나를 만드는 흐름이 있어도 목록에 입구가 없어, "페르소나 골라줘" 로는 도달할 수 없었습니다. 이제 목록에 항상 표시되며, 고르면 질문 몇 개로 바로 만들 수 있습니다. 페르소나가 많아 목록이 길어지면 갈래로 나눠 보여주고, 어떤 경우에도 만들어 둔 페르소나가 목록에서 빠지지 않습니다.
- **페르소나를 바꾸면 그 자리에서 바로 바뀝니다.** 이전에는 세션 도중에 바꿔도 다음 세션 전까지 이전 캐릭터의 말투·호칭이 남아 있고, 인사말도 엉뚱하게 나오는 경우가 있었습니다. 이제 바꾸는 즉시 새 캐릭터를 적용합니다.
- **페르소나를 만들 때 인사말을 직접 고르거나 쓸 수 있습니다.** 후보 몇 개를 보여드리고, 마음에 드는 걸 고르거나 직접 쓸 수 있습니다.
- **코드 리뷰(`/codex-review`)가 테스트를 돌리는 중에 잘못 중단되지 않습니다.** 리뷰 도구가 명령을 실행하는 동안에는 출력이 멈춰 있는데, 이전에는 이를 "멈춤" 으로 오해해 정상 진행 중인 리뷰를 중단시켰습니다 — 테스트를 성실히 돌리는 리뷰일수록 중단되는 문제였습니다. 이제 명령 실행 중인지, 자식 프로세스가 살아 있는지까지 함께 보고 판단합니다. 진짜로 멈춘 리뷰는 여전히 감지해 대체 리뷰로 넘깁니다. 조용한 시간 허용치도 60초에서 180초로 늘어, 깊게 생각하는 구간이 중단되지 않습니다.

## v1.6.3 — 2026-07-23 (페르소나 중립 기본 + 선택·커스텀·전환 인사말·언어 적응 + 코드리뷰 시간상한)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **페르소나 기본값이 중립으로 바뀝니다.** 이전에는 별도 설정이 없으면 boss-ace 페르소나가 자동으로 켜졌지만, 이제 기본은 담백한 중립 말투입니다. `.rein/policy/persona.yaml` 에 명시적으로 `enabled: true` 를 적은 경우에만 페르소나가 활성화됩니다 — 손으로 만든 설정 파일이라도 `enabled` 키가 없으면 중립으로 처리됩니다(엄격화).
- **내장 페르소나 `jennie` 가 추가됩니다.** 기존 `boss-ace` 와 함께 내장 2종 중에서 고를 수 있습니다.
- **persona 스킬이 추가됩니다.** 세션에서 "페르소나 골라줘" 라고 말하면 내장 페르소나 목록에서 고르거나, 질문 몇 개에 답해 나만의 커스텀 페르소나를 만들 수 있습니다.
- **페르소나를 바꾸면 그 캐릭터의 인사말 한 줄이 먼저 뜹니다.** 페르소나를 고르거나 바꾸는 순간, 그 페르소나다운 짧은 인사 한 줄이 결과 보고 앞에 표시돼 전환이 바로 체감됩니다. 중립으로 끌 때는 담백한 한 줄로 알려줍니다.
- **인사말은 대화 언어에 맞춰 나옵니다.** 영어로 대화하는 중에 페르소나를 바꾸면 인사말도 영어로 나옵니다 — 저장된 인사말은 그대로 두고 대화 언어에 맞춰 같은 뜻으로 표현합니다. 한국어 대화면 한국어 그대로입니다.
- **코드 리뷰(`/codex-review`)가 멈추면 자동으로 감지해 넘어갑니다.** 리뷰가 시간 상한을 넘겨도 응답이 계속 자라는 동안은 기다리지만, 일정 시간 출력이 전혀 늘지 않으면(멈춤 판정) 자동으로 중단하고 대체 리뷰로 전환합니다. 리뷰가 무한정 매달려 세션을 잡아먹는 일을 막습니다.
- **기존 boss-ace 말투를 유지하려면** 아래 한 줄로 복원됩니다:

```bash
mkdir -p .rein/policy && printf 'enabled: true\npreset: boss-ace\n' > .rein/policy/persona.yaml
```

> 버전 등급 메모: Rule A 기계 판정은 minor(새 페르소나 기능 다수 + 코드리뷰 시간상한 워치독). **사용자 결정으로 patch(1.6.3) 릴리스** — 결정 주체와 기계 판정을 모두 기록해 둔다.

## v1.6.2 — 2026-07-22 (리뷰 전에 로컬 검증 통과를 요구 + 통과 항목 서술 축소)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **코드 리뷰(`/codex-review`)가 리뷰 전에 로컬 검증 증거를 요구합니다.** 변경(스테이징·미스테이징·신규 파일)이 있는데 — 또는 변경 여부 확인 자체가 실패해 안전하게 "변경 있음"으로 간주되는데 — 로컬 검증 증거 없이 리뷰를 요청하면, 리뷰 비용을 쓰기 전에 즉시 거부하고 무엇이 필요한지 알려줍니다(전용 exit 4). 증거는 기존 `[EVIDENCE]` 블록 문법 그대로 두 축 — 타입체크/문법검사 성공(claim 에 `[axis:typecheck]`)과 테스트 성공(`[axis:test]`) — 에 자기 diff 검토 한 줄(`diff_self_review:`)을 더한 형태입니다. 검증 명령이 없는 프로젝트는 `verification_commands: none` 선언으로, TDD 의 의도된 실패(red) 단계는 `verification_state: tests-intentionally-red` + `expected_failure: <이름>` 선언으로 통과합니다(실패 종료코드가 timeout·명령없음·시그널이면 의도된 red 로 인정하지 않음). 목적: 로컬에서 잡혔을 결함으로 리뷰 라운드를 낭비하는 "고침 → 새 문제 → 재리뷰" 루프 차단. 설계·계획 문서 리뷰는 대상이 아닙니다.
- **리뷰 응답이 짧아집니다.** 통과한 항목은 "검사 N개 / 통과 N개" 한 줄 요약으로 줄이고, 지적 사항만 전량 상세히 서술합니다. 입력이 없는 섹션은 생략됩니다. 감사 목적으로 전체 서술이 필요하면 `REIN_REVIEW_VERBOSE=1` 로 이전 형식을 복원할 수 있습니다. 판정 방식과 통과 기록은 그대로입니다.

> 버전 등급 메모: Rule A 기계 판정은 minor(신규 관문 — 증거 없는 리뷰 호출은 새로 거부됨). **사용자 결정으로 patch(1.6.2) 릴리스** — 결정 주체와 기계 판정을 모두 기록해 둔다.

## v1.6.1 — 2026-07-13 (리뷰 요청서에 재현 증거 요구 + 상태 요약 길이를 작성 시점에 강제)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **코드 리뷰(`/codex-review`) 요청서에 수치·통과 주장을 쓰면 재현 증거 블록이 필요합니다.** "테스트 N건 통과" 같은 주장을 증거(`[EVIDENCE]` 블록: 주장/재현 명령/종료코드/출력 발췌) 없이 쓰면 래퍼가 리뷰 비용을 쓰기 전에 즉시 거부하고 무엇이 빠졌는지 알려줍니다(전용 exit 4). 증거 블록이 전혀 없는 정량 요청서는 사전에 차단되고, 블록이 있는 요청서의 블록 밖 잔존 수치는 경고와 함께 리뷰어에게 표시됩니다. 증거는 리뷰어에게 구조화되어 전달돼 판정도 짧아집니다. 수치 주장이 없는 기존 요청서는 완전히 그대로 동작합니다(하위호환 — 봉투가 바이트 단위로 동일함을 테스트로 고정).
- **상태 요약 문서(trail/index.md)의 5~25줄 규칙이 편집 시점에 바로 강제됩니다.** 이전에는 세션 종료 때 적발돼 다시 줄이는 왕복이 있었는데, 이제 초과하는 편집 자체가 즉시 거부되고 압축 안내가 나옵니다. 세션 종료 검사는 이중 방어로 유지됩니다.
- **편집 때마다 쌓이던 진단 기록 파일(`trail/inbox/*-meta-check.jsonl`)이 더 이상 생성되지 않습니다.** 읽는 곳이 없는 순수 부산물이었습니다. 기존에 쌓인 파일은 직접 삭제해도 안전합니다. 편집-작업문서 불일치 경고 기능 자체는 그대로입니다.

> 버전 등급 메모: Rule A 기계 판정은 minor(신규 게이트 동작 2건 — 증거 없는 정량 요청서는 새로 거부됨). **사용자 결정으로 patch(1.6.1) 릴리스** — 결정 주체와 기계 판정을 모두 기록해 둔다.

## v1.6.0 — 2026-07-10 (리뷰 모델이 최신 codex 세대로 업그레이드 + 질문 성격별 모델 자동 선택)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **코드 리뷰(`/codex-review`)가 최신 codex 프론티어 모델(gpt-5.6-sol)로 심사합니다.** 판정 일관성을 위해 리뷰 모델은 하나로 고정하고, 검토 강도만 변경 규모에 맞춰 자동 조절하는 기존 방식은 그대로입니다.
- **독립 의견(`/codex-ask`)은 질문 성격에 따라 모델을 자동 선택합니다.** 기계적인 추출·분류는 빠르고 저렴한 모델, 일반 질문은 균형 모델, 고위험·모호한 판단은 가장 깊은 모델 — 비용과 속도가 질문에 맞춰집니다.
- **codex 모델 설정 파일이 없어도 리뷰가 예측 가능한 기본 모델로 실행됩니다.** 이전에는 설정이 없으면 모델 지정 없이 호출되어 codex 쪽 기본값에 좌우됐습니다. 이제 내장 기본 모델로 명시 실행되고 경고를 한 줄 남깁니다.
- **보안·훅·설정 경로를 건드린 변경은 작아도 가장 가벼운 강도로 검토되지 않습니다.** 변경 크기와 위험도는 다르므로, 민감 경로가 포함되면 검토 강도에 하한선을 둡니다. 직접 강도를 지정하면 그 값이 여전히 우선합니다.
- **리뷰 통과 기록에 어떤 모델·강도로 심사했는지 남습니다.** 나중에 리뷰 품질을 되짚거나 모델을 바꿀 때 근거 자료가 됩니다.
- **codex 의 새 검토 강도(ultra/max/xhigh)는 리뷰 게이트에서 사유와 함께 거부됩니다.** ultra 는 작업을 자동 분할하는 모드라 게이트 판정의 일관성과 맞지 않고, max/xhigh 는 소요 시간 실측 후 지원을 검토합니다.

> 버전 등급 메모: 리뷰 게이트 모델 변경 + `/codex-ask` 라우팅 신설 + 통과 기록 확장 → user-facing 신규 기능 → minor(1.6.0) (versioning Rule A).

## v1.5.8 — 2026-06-26 (코드 리뷰 강도가 변경 규모에 맞춰 자동 조절)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **코드 리뷰(`/codex-review`)가 변경 규모에 맞춰 빨라집니다.** 이전에는 자동 리뷰가 거의 항상 가장 깊은(=가장 느린) 강도로 돌아 작은 변경에도 시간이 오래 걸렸습니다. 이제 변경된 코드 양(줄 수·파일 수)과 문서 길이를 보고 작은 변경은 가볍게, 큰 변경만 깊게 검토하도록 강도를 자동으로 정합니다. 직접 강도를 지정하면 그 값이 우선하고, 측정이 어려우면 안전하게 가장 깊은 강도로 둡니다.

## v1.5.7 — 2026-06-18 (코드 리뷰 도우미 스크립트 호출 경로 수정)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **코드 리뷰(`/codex-review`)와 설계 리뷰가 어느 폴더에서 실행하든 설치된 플러그인의 도우미 스크립트를 정확히 찾습니다.** 이전에는 관련 안내 문서가 도우미 스크립트를 현재 폴더 기준 상대 경로(`scripts/...`)로 호출하도록 적혀 있어서, 그 폴더가 없는 사용자 저장소에서는 호출이 어긋날 수 있었습니다. 이제 설치된 플러그인 경로를 먼저 쓰고, 없을 때만 현재 폴더로 물러나도록 바로잡았습니다.

> 버전 등급 메모: 사용자에게 ship 되는 스킬·규칙·에이전트 안내의 호출 경로 수정 → user-facing → patch(1.5.7) (versioning Rule A).

## v1.5.6 — 2026-06-17 (보안과 무관한 커밋은 보안 재검토 자동 면제)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **문서·운영기록·버전 문자열만 바꾼 커밋은 보안 재검토 없이 통과합니다.** 이전에는 보안 검토가 코드 리뷰보다 같거나 새 것이어야 한다는 요구(v1.5.5) 때문에, 릴리스나 문서 정리처럼 보안과 전혀 무관한 커밋도 코드 리뷰가 새로 돌면 보안 검토를 한 번 더 받아야 통과했습니다. 이제 그 커밋에 담긴 변경이 전부 문서(`*.md`/`docs/`), 운영기록(`trail/`), 또는 정해진 두 파일의 버전 문자열(설치 스크립트의 버전 줄 / 플러그인 매니페스트의 최상위 version 값)뿐이면 보안 재검토를 자동으로 건너뜁니다.
- **진짜 코드·설정 변경은 예전처럼 계속 보안 검토를 요구합니다.** 면제는 위 허용 목록에 정확히 들어맞을 때만이고, 목록 밖 파일이 하나라도 섞이면(소스 코드, 훅 로직, 임의의 설정 파일 등) 보안 검토를 그대로 요구합니다 — 안전한 쪽이 기본값입니다.
- **면제는 평범한 단독 커밋에만 적용됩니다.** 같은 명령줄에 파일을 추가로 올리거나(`git add ... && git commit`), 다른 명령을 끼우거나(앞뒤에 `cd`/`command`/`env` 등), 백그라운드 실행·다른 저장소 지정 같은 형태면 면제하지 않고 보안 검토를 요구합니다(커밋 직전에 내용이 바뀔 여지를 차단).
- **면제가 발동하면 기록을 한 줄 남깁니다.** 어떤 커밋이 보안 검토를 면제받았는지 운영 로그에서 확인할 수 있습니다.

> 버전 등급 메모: hook 동작 변경(보안 검토 요구 완화 — 단, 허용 목록 외 전부 보안 요구 = fail-closed) → user-facing → patch(1.5.6) (versioning Rule A). v1.5.5 의 보안 신선도 검사가 낳은 릴리스 마찰의 근본 해결. 위협모델은 "정직한 에이전트 규율"까지 — 의도적 우회 하드닝은 범위 밖.

## v1.5.5 — 2026-06-16 (리뷰·보안 검토가 조용히 통과되던 구멍 봉합 + 설계 검증기 호환성)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **보안 검토가 코드 리뷰보다 오래됐으면 커밋이 차단됩니다.** 이전에는 보안 검토 기록이 "있기만" 하면 통과해서, 코드를 새로 고치고 코드 리뷰만 다시 받아도 예전 보안 검토가 그대로 인정됐습니다. 이제 보안 검토가 코드 리뷰보다 같거나 새 것이고 같은 작업 사이클일 때만 통과합니다(작업 정의가 가벼운 보안 등급으로 면제된 경우는 예외).
- **기록된 코드 리뷰가 실제 "통과"가 아니면 커밋이 차단됩니다.** 이전에는 기록 존재만 확인해, 수정 필요·사람 확인 대기 상태의 기록으로도 커밋이 됐습니다. 이제 통과 판정을 확인합니다(이전·현재 기록 형식 모두 인식).
- **설계 리뷰 대기 표시 생성기가 실패하면 다음 편집을 보수적으로 차단합니다.** 이전에는 Python/JSON 처리 실패 시 조용히 통과해 미리뷰 설계 변경이 빠져나갈 수 있었는데, 이제 차단하고 원인 해소 후 자동 해제합니다(1회용 비상 우회 제공, 우회는 한 번 쓰면 소비).
- **설계 리뷰 1회 우회 표시가 한 번 쓰면 소비됩니다.** 이전에는 손으로 지울 때까지 게이트가 계속 꺼졌는데, 이제 1회 사용 후 자동 삭제됩니다(제거 실패 시 차단).
- **설계 문서를 손으로 형식 맞출 필요가 줄었습니다.** 커버리지 검증기가 번호 붙은 제목(`## 3. Scope Items`)과 백틱으로 감싼 항목 ID를 그대로 인식합니다(이전에는 못 읽어 검증을 통째로 건너뛰던 문제 포함).

> 버전 등급 메모: hook·검증기 동작 변경(차단 조건 강화 + 호환성) → user-facing 버그 수정/하드닝 → patch(1.5.5) (versioning Rule A). 위협모델은 "정직한 에이전트 규율"까지 — 의도적 우회 하드닝은 범위 밖.

## v1.5.4 — 2026-06-15 (자동모드 안내 정정 + 편집 중 거짓 불일치 경고 제거)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **자동모드를 켜도 "작업 기록(인덱스/inbox) 미갱신" 종료 차단은 그대로 발동한다는 점을 안내가 정확히 설명합니다.** 이전 자동모드 설명은 "세션 종료 차단이 모두 조용해진다"고 했지만, 실제로는 반복 문제(incident) 관련 차단만 조용해지고 trail 기록 확인 차단은 자동모드여도 켜져 있습니다 — 동작 변경 없이 설명만 실제와 일치하도록 고쳤습니다(자율 작업 중에도 세션 종료 전 trail 기록은 갱신해야 합니다).
- **파일을 편집할 때마다 뜨던 "변경 파일 수가 작업 정의와 안 맞음" 거짓 경고를 줄입니다.** 저장소에 오래 방치된, 버전 관리 밖 파일(예: 데이터·이미지 부산물)이 매번 변경 목록에 섞여 들어가 경고를 유발했는데, 이제 이번 세션에 실제로 새로 만들어진 파일만(세션 시작 이후 수정된 것만) 대상으로 봅니다. 판단 기준을 못 읽는 환경에서는 예전처럼 전체를 대상으로 둬 새 파일 누락은 없습니다.

> 버전 등급 메모: hook 동작 변경(거짓 경고 제거) → user-facing 버그 수정 + 사용자 노출 스킬 안내 정정 동봉 → patch(1.5.4) (versioning Rule A). 발단은 외부 사용자 제보 → 독립 검증.

## v1.5.3 — 2026-06-12 (게이트가 조용히 안 켜지던 구멍 4건 봉합)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **커밋 전 검사가 더 많은 형태의 커밋에서 제대로 작동합니다.** 이전에는 `git -C <폴더> commit`, 공백이 두 칸 들어간 형태, 일부 옵션이 붙은 형태로 커밋하면 리뷰·보안 검사가 조용히 건너뛰어졌습니다. 이제 이런 형태도 동일하게 검사가 걸립니다(`git commit-graph` 같은 별개 명령은 정확히 제외).
- **커밋 메시지에 "git merge" 같은 단어가 들어갔다는 이유로 검사가 통째로 건너뛰던 문제를 수정합니다.** 진짜 병합·리베이스 커밋은 예전처럼 면제되지만, 메시지에 그 단어가 든 일반 커밋은 더 이상 검사를 우회하지 않습니다.
- **"설계 먼저, 코드 나중" 요구가 더 많은 언어·구조에 적용됩니다.** 이전에는 몇몇 디렉토리 이름(`src`, `app`, `lib` 등)으로만 소스 파일을 인식해 Go 의 `internal`·`cmd`, 저장소 루트의 소스 파일 등이 통째로 빠져나갔습니다. 이제 소스 확장자로도 인식하며(문서·데이터·잠금 파일은 계속 제외), 새로 적용되는 파일을 처음 편집할 때 이유를 한 번 안내합니다.
- **Python 이 바로 잡히지 않는 환경에서 게이트가 스스로 꺼지던 문제를 수정합니다.** 이전에는 인터프리터를 못 찾으면 검사를 "꺼짐"으로 오인해 통과시켰는데, 이제 그런 경우 게이트를 켜진 상태로 유지합니다(fail-closed).

> 버전 등급 메모: hook 동작 변경(차단 조건) → user-facing 버그 수정 → patch(1.5.3) (versioning Rule A). 위협모델은 "정직한 에이전트 규율"까지 — 의도적 우회 하드닝은 범위 밖.

## v1.5.2 — 2026-06-11 (코드 리뷰 오탐 감소 + 리뷰 게이트 구멍 봉합)

`rein update` 후 사용자 세션에서 바뀌는 것:

- **커밋 전 코드 리뷰의 "엉뚱한 설계 기준" 오탐이 줄어듭니다.** 명시적인 작업 표식 없이 리뷰를 돌리면 시스템이 가장 최근 작업 문서를 추측으로 골라 쓰는데, 이제 그 추측을 확정 기준처럼 들이대지 않고 "참고용"으로 정직하게 표시합니다 — 무관한 작업의 설계 기준으로 "설계 누락/모순" 차단 경고를 내던 오탐이 사라집니다. 리뷰 자료의 시각 정보도 리뷰 대상(커밋 전 작업 / 커밋 범위 / 설계 문서)에 맞게 정직해집니다.
- **정상 작업 표식이 세션 시작마다 지워지던 문제를 수정합니다.** 설계 연결 섹션이 없는(정상적인) 작업 문서를 가리키는 표식을 청소 로직이 잘못 삭제해, 위의 "추측 모드"로 떨어지던 근본 원인이었습니다.
- **리뷰 도구의 신뢰성 구멍 2건을 봉합합니다.** (1) 리뷰가 통과로 끝났는데도 출력이 크면 모델 오류로 오인해 통과 표시를 만들지 못하던 문제, (2) 아주 큰 설계 리뷰 요청이 코드 리뷰로 오분류되어 **코드 게이트 통과 표시를 잘못 만들 수 있던** 문제 — 둘 다 같은 원인 계열로, 재현 테스트와 함께 수정됐습니다.

> 버전 등급 메모: hook/리뷰 도구 오작동 수정 → patch(1.5.2) (versioning Rule A). 같은 날 v1.5.1 이후 추가 배포는 게이트 무결성 구멍(설계 리뷰의 코드 게이트 오염 가능성) 핫픽스 명분 + 사용자 결정으로 Rule B(같은 날 1버전) override — hotfix for v1.5.1.
