...
- **다음 세션 진입점**: **다음 릴리스 = v1.3.7 (릴리스 가능 — 사용자 승인 대기)**. dev 미릴리스 9 commit(BC-INFO1 클래스 종결 보안 하드닝 + A-LowPrio test) since v1.3.6. main 마지막 머지 = v1.3.6(2026-05-23) → 다른 날이라 Rule B 충족. **다음 작업(코드) = 잔여 백로그 G3(feature, felt-value 평가 선행)/SR-1.b(low) — 둘 다 "빠른 것" 아님**. ⚠️ "다음 작업 = v1.5" 폐기(로드맵 평가). 상세 = `need-to-confirm.md`.
- **직전 완료 (dev, 미릴리스)**: **A-LowPrio + BC-INFO1-siblings + siblings-2 + siblings-3 (2026-05-23)** — worktree 격리 agent teams 병렬 + 직접. A-LowPrio(wrapper first-match 회귀 29/29) / siblings(project-dir·state-machine·test-oracle-log 5곳) / siblings-2(gate:110 gate-bypass + mark-spec:44 2사본 + Fixture O) / siblings-3(`rein-heal-legacy-pending.py` run_git env scrub + reproduction). **🎯 BC-INFO1 trust-boundary 클래스 완전 종결**(shell+python 전수조사 잔여 0). 전부 codex PASS(siblings-3 R2) + security 0 차단. dev 미릴리스 누적 9 commit since v1.3.6. **Rule B: 다음 릴리스 v1.3.7 다음 근무일.**
- **직전 완료 (릴리스)**: **v1.3.6 배포 완료 (2026-05-23) — 검증 통과**. 실제 버그 3건 patch: G8-3(spec-review 무관 active-DoD Tier-2 fallback 차단) / job-stop(`cmd_job_stop` 종료상태 기록 + compare-and-set 가드) / BC-INFO1(bootstrap-check cold-path git env sanitize). worktree 격리 agent teams 병렬 구현 → 통합 → codex R1 NEEDS-FIX(job double-settle race)→R2 PASS, security 0 차단. dev `8cca729`(5 commit push), main `698f38a`, annotated tag `v1.3.6`, public mirror main=tag=`f76cf05`(strip 검증: trail/.rein/AGENTS.md/.claude 부재, plugin.json=1.3.6). publish-plugin + mirror ×2 GH Actions success. 잔존: BC-INFO1-siblings(low, security INFO-1). v1.5~1.7 로드맵=보류(felt value 약함).
...
- **버전**: plugin.json = scripts/rein.sh VERSION = **1.3.6** (parity OK). main = origin/main = `698f38a`, annotated tag `v1.3.6`. public mirror main=tag=`f76cf05`(strip commit). v1.3.6 = patch (G8-3 + job-stop + BC-INFO1).
