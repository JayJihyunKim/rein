...
run_wrapper() {
  ...
  # sandbox 는 untracked 하네스 부산물 투성이라 관문이 발동한다 — 이 스위트의
  # 검증 대상은 readiness 사전검사/manifest 이므로 none 폴백 선언으로 통과
  # (관문 자체 계약은 test-review-selfverify-gate.sh 소유). E5b 기준 래퍼
  # 비교는 양쪽에 같은 stdin 이 가므로 대칭 유지.
...
if [ -s "$SANDBOX/scripts/base-wrapper.sh" ]; then
  ...
  (
    ...
    export TMPDIR="$SANDBOX/tmpdir"
    # run_wrapper 와 동일한 stdin (fixture 통행증 포함) — E5b 대칭 비교 유지.
    printf '%s\nverification_commands: none\ndiff_self_review: harness fixture pass\n' \
  ...
  if [ -f "$BASE_CAPTURE" ] && [ -f "$CAPTURE" ]; then
    ...
    if diff -q "$SANDBOX/.norm-base.txt" "$SANDBOX/.norm-new.txt" >/dev/null 2>&1; then
      echo "  ok: E5b 무주장 요청서 envelope 이 기준 래퍼와 동일 (출력 밀도 블록 정규화 후)"
    else
      fail "E5b envelope 이 기준 래퍼와 다름: $(diff "$SANDBOX/.norm-base.txt" "$SANDBOX/.norm-new.txt" | head -5)"
    fi
  else
    fail "E5b 캡처 파일 누락"
  fi
