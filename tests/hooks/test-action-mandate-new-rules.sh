#!/usr/bin/env bash
# Verify plugins/rein-core/rules/{answer-only-mode,subagent-review,background-jobs}.md
# each have a `## 행동 강령` section as the FIRST `## ` header, with body size <= 2048 bytes.
set -e
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
cd "$PROJECT_DIR"

RULES=(answer-only-mode subagent-review background-jobs)
MAX=2048
FAIL=0
...
[ "$FAIL" = "0" ] || { echo "test-action-mandate-new-rules: FAIL" >&2; exit 1; }
echo "test-action-mandate-new-rules: OK"
