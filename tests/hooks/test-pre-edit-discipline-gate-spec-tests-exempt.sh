#!/bin/bash
# tests/hooks/test-pre-edit-discipline-gate-spec-tests-exempt.sh
#
# Phase 7 웨이브 3 ③-b (편집 게이트 교대): pre-edit-dod-gate.sh 삭제 후
# spec-review-gate 축은 pre-edit-discipline-gate.sh 가 이어받는다. tests/ 면제는
# 활성작업(active-task) 축과 무관하므로 파일명만 새 훅으로 rename, 로직/단언은
# 불변.
#
# DOD-GATE-FP-TESTS — spec review gate must not block edits to tests/.
#
# Bug class: the spec review gate (pre-edit-discipline-gate.sh) sets
...
run_test test_tests_path_allowed_with_unreviewed_spec pre-edit-discipline-gate.sh
run_test test_tests_scripts_path_allowed_with_unreviewed_spec pre-edit-discipline-gate.sh
run_test test_non_tests_source_still_blocked_with_unreviewed_spec pre-edit-discipline-gate.sh
run_test test_src_file_still_blocked_with_unreviewed_spec pre-edit-discipline-gate.sh
run_test test_tests_substring_not_under_tests_dir_still_blocked pre-edit-discipline-gate.sh
