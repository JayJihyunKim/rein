#!/bin/bash
# tests/hooks/test-pre-edit-dod-gate-repo-scope.sh
...
- slug: unrelated-work'
  ...
  run_hook pre-edit-dod-gate.sh "$(_edit_scripts_input)"
...
run_test test_outside_repo_marker_does_not_block pre-edit-dod-gate.sh
run_test test_outside_repo_marker_not_deleted pre-edit-dod-gate.sh
run_test test_related_unreviewed_spec_still_blocks pre-edit-dod-gate.sh
run_test test_related_via_plan_ref_still_blocks pre-edit-dod-gate.sh
run_test test_related_via_annotated_plan_ref_still_blocks pre-edit-dod-gate.sh
run_test test_outside_repo_orphan_reviewed_marker_skipped pre-edit-dod-gate.sh
run_test test_internal_symlink_to_outside_marker_skipped pre-edit-dod-gate.sh
# 1계층 선택 표식이 있으면 게이트가 선택 기준서에 커버리지 검증기를 실행한다
# — 샌드박스에 검증기 스크립트를 함께 복사해야 별개 사유(검증기 부재 fail-
# closed)로 차단되지 않는다.
run_test test_other_active_dod_reference_warns_when_selected_dod_unrelated pre-edit-dod-gate.sh rein-validate-coverage-matrix.py
run_test test_unrelated_unreviewed_spec_warns_not_blocks pre-edit-dod-gate.sh
run_test test_mixed_related_and_unrelated_blocks pre-edit-dod-gate.sh
run_test test_related_reviewed_spec_passes pre-edit-dod-gate.sh
run_test test_completed_dod_reference_does_not_block pre-edit-dod-gate.sh
run_test test_tests_exempt_survives_related_block pre-edit-dod-gate.sh
