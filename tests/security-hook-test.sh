...
setup_project "$proj1"
touch "$proj1/trail/dod/dod-test-task.md"
touch "$proj1/trail/dod/.codex-reviewed"
...
setup_project "$proj2"
touch "$proj2/trail/dod/dod-test-task.md"
touch "$proj2/trail/dod/.codex-reviewed"
touch "$proj2/trail/dod/.security-reviewed"
...
setup_project "$proj3"
touch "$proj3/trail/dod/dod-test-task.md"
touch "$proj3/trail/dod/.codex-reviewed"
# NO .security-reviewed stamp
...
# ---------------------------------------------------------------------------
echo ""
echo "Test: old security stamp still allowed (TTL removed in v0.6.0)"
proj5="$TEST_DIR/proj5"
setup_project "$proj5"
touch "$proj5/trail/dod/dod-test-task.md"
touch "$proj5/trail/dod/.codex-reviewed"
touch "$proj5/trail/dod/.security-reviewed"
# Make stamp 2 hours old — no longer matters (TTL removed in v0.6.0 A1)
touch -t "$(date -v-2H +%Y%m%d%H%M.%S 2>/dev/null || date -d '2 hours ago' +%Y%m%d%H%M.%S 2>/dev/null)" "$proj5/trail/dod/.security-reviewed"

set +e
run_hook "$proj5" "pytest tests/ -v"
exit5=$?
set -e
assert_exit_code "pytest allowed with old security stamp (no TTL)" 0 "$exit5"
