...
Latest release: **v2.1.0** (2026-09-07) — three user-facing changes: a review request that still carries unbacked result claims outside its evidence blocks is now rejected before codex runs, with the offending line and a rewrite hint — a "tests pass" sentence is always rejected; a ratio or percentage is rejected when it has no contract word (limit/target/threshold) or carries a result verb ("85% coverage", "3/5 failed above the limit"), while "±20% tolerance" stays a warning; a count with a unit is rejected only when it has a result verb and no contract word ("2 files flagged", "3 tests failed"), while "50 lines max", "5 files" and "at most 3 checks failed" stay warnings (warnings apply when at least one valid evidence block is present; with no block at all, every unbacked claim is rejected as before); the rejection costs no review round. The comment rule now keeps only lasting contracts in code — round numbers, fix history, and measured results go to `trail/`, reviewers check only added or modified comment lines, and stale narrative is deleted or moved rather than corrected. Docs-only review rounds skip the typecheck/test evidence requirement when every changed path observed now was already in the review-start snapshot (a path-set containment check, not a content comparison; `--print-subject` now also reports that snapshot as `changeset_paths`, and the security review output is unchanged). ([CHANGELOG](CHANGELOG.md))

Previous: **v2.0.3** (2026-09-04) — four user-facing fixes: the security review gate no longer silently disappears when a project has no local policy; non-git folders get one consistent onboarding path (Rein requires a git repository, asks before `git init`, `--allow-non-git` stays an explicit opt-in); Rein's runtime state files are added to `.gitignore` so review fingerprints stop drifting; and ending a session in a never-bootstrapped project leaves no `trail/` residue. ([CHANGELOG](CHANGELOG.md))
