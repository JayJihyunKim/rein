...
Latest release: **v2.1.1** (2026-09-08) — three user-facing fixes: a subagent working in the shared tree can no longer run a mutating `git stash` (the Bash guard denies it only when the hook input carries a subagent id; `stash list`/`show` and the main session are untouched); the guard's clause-start detection now also catches invocations inside `if … then`, `while … do`, `case … )`, brace groups and backtick substitutions for the four rules that share the classifier (`.env` read, `.env` commit, destructive-git confirmation, subagent stash) — a plain mention placed right after such a token may be denied on the conservative side, so rephrase and rerun; and the bootstrap script preserves non-UTF-8 `.gitignore` bytes instead of crashing and refuses a directory sitting at `.rein/project.json`. ([CHANGELOG](CHANGELOG.md))
