...
Latest release: **v2.1.3** (2026-09-22) — a follow-up patch to v2.1.2, whose marketplace publish was blocked when the macOS preflight exposed an awk-portability defect: with an awk that does not fold backslashes in replacement strings (macOS's default awk, BusyBox awk) the new review-events log double-escaped backslashes inside recorded values (the JSON stayed well-formed; typical Linux and WSL2 setups using gawk or mawk were unaffected). Everything from v2.1.2 ships in this version — an opt-in "main session conducts" rule: add `orchestrator-first:` with `enabled: true` to your project's `.rein/policy/rules.yaml` and, from the next session, the session-start briefing tells the main session to decompose non-trivial development work (features, bug fixes, refactors), delegate independent units to worker subagents in parallel, and review once per wave instead of once per worker. It is off by default — the rule summary is not added to the session-start briefing unless you add the entry, and a broken or unreadable policy file never turns it on. Independently of that setting, the `rein:orchestrator` agent is now described as an explicit-call option path rather than the default entry point, and the three Builder agents' completion criteria distinguish a standalone run from a worker dispatch, where the parent runs the code review and (unless exempted) the security review at the end of the wave. Also in this release: every `/codex-review` call that ends in a verdict appends one line (time, review kind, verdict, round, wall-clock duration) under `trail/review-events/` that survives a pass, and the skill guide gains a follow-up request form for second and later review rounds, since the wrapper starts a fresh reviewer session each time. ([CHANGELOG](CHANGELOG.md))

Previous: **v2.1.0** (2026-09-07) — three user-facing changes: a review request with unbacked result claims outside its evidence blocks is rejected before codex runs (with the offending line and a rewrite hint, no review round spent); the comment rule keeps only lasting contracts in code; docs-only review rounds skip the typecheck/test evidence requirement when every changed path was already in the review-start snapshot (`changeset_paths`). ([CHANGELOG](CHANGELOG.md))
